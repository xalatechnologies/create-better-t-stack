...
export function validateServiceMetadata(metadata: any): boolean {
  ...
  if (!metadata.category || !Object.values(SERVICE_CATEGORIES).includes(metadata.category)) {
    return false;
  }
...
export function validateSOLIDCompliance(service: any): {
  ...
  if (typeof service.name !== 'string' || !service.name) {
    violations.push('SRP: Service must have a clear name indicating its responsibility');
  }
  ...
  if (service.constructor && service.constructor.prototype) {
    ...
    if (!proto.initialize || typeof proto.initialize !== 'function') {
      suggestions.push('LSP: Consider implementing initialize() method for consistent behavior');
    }
  }
  
  // Interface Segregation Principle
  const methodCount = Object.getOwnPropertyNames(service)
    .filter(prop => typeof service[prop] === 'function').length;
  
  if (methodCount > 20) {
    violations.push('ISP: Service has too many methods, consider splitting into smaller interfaces');
  }
  ...
  try {
    ...
    for (const [serviceId, isHealthy] of healthResults) {
      ...
      if (!isValidServiceId(serviceId)) {
        warnings.push(`Service ID does not follow naming convention: ${serviceId}`);
      }
    }
    ...
    const servicesByCategory = factory.getStatistics().containerStats.categoryCounts;
    ...
    if (totalServices > 50) {
      suggestions.push('Consider organizing services into modules for better performance');
    }
