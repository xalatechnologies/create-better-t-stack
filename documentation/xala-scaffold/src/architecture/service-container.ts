...
export class ServiceContainer {
	...
	register<T>(
		...
		if (this.registrations.has(id)) {
			logger.warn(
				`Service ${id} is already registered, replacing existing registration`,
			);
		}
	...
	createScope(scopeId?: string): string {
		...
		const id =
			scopeId ||
			`scope_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
		...
		for (const [serviceId, instance] of scope.services) {
			if (instance && typeof instance.dispose === "function") {
				try {
					...
					logger.debug(
						`Disposed scoped service: ${serviceId} in scope ${scopeId}`,
					);
				} catch (error) {
			...
			for (const [serviceId, instance] of singletons.reverse()) {
				if (instance && typeof instance.dispose === "function") {
					try {
						...
						logger.error(
							`Failed to dispose singleton service ${serviceId}:`,
							error,
						);
					}
				}
	 ...
	 */
	private createTransientInstance<T>(
		id: string,
		registration: ServiceRegistration<T>,
	): T {
	...
	private configureInstance(instance: any): void {
		...
		if (
			...
			typeof instance.setProgressReporter === "function"
		) {
