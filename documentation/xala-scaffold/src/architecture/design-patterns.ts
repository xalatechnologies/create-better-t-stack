...
import { IEventEmitter, IEventListener, IProgressReporter } from './interfaces.js';
...
export class StrategyContext<TInput, TOutput> {
   ...
   */
  async executeStrategy(name: string, input: TInput, context?: any): Promise<TOutput> {
...
export class LoggingDecorator<T> extends BaseDecorator<T> {
  ...
  async execute(input: T): Promise<T> {
    ...
    try {
      ...
      logger.debug(`Completed ${this.operationName} in ${duration.toFixed(2)}ms`);
      return result;
    } catch (error) {
      const duration = performance.now() - startTime;
      logger.error(`Failed ${this.operationName} after ${duration.toFixed(2)}ms:`, error);
      throw error;
    }
  }
}
 ...
 */
export abstract class AbstractHandler<TRequest, TResponse> implements IHandler<TRequest, TResponse> {
...
export abstract class Singleton<T> {
  ...
  protected constructor() {
    ...
    if (Singleton.instances.has(constructor)) {
      throw new Error(`Singleton instance already exists for ${constructor.name}`);
    }
  }
