...
export interface IMigrationService extends IBaseService {
  ...
  generateMigrationPlan(analysis: IProjectAnalysis, targetPlatform: string): IMigrationPlan;
}
...
export interface ILocalizationService extends IBaseService {
  translate(key: string, locale: LocaleCode, params?: Record<string, any>): string;
  hasTranslation(key: string, locale: LocaleCode): boolean;
  loadTranslations(locale: LocaleCode, translations: Record<string, string>): void;
  getSupportedLocales(): LocaleCode[];
  detectLocale(request?: any): LocaleCode;
  formatNumber(value: number, locale: LocaleCode, options?: Intl.NumberFormatOptions): string;
  formatDate(date: Date, locale: LocaleCode, options?: Intl.DateTimeFormatOptions): string;
