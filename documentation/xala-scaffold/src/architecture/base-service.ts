import { IBaseService, IEventEmitter, IProgressReporter } from './interfaces.js';
...
export abstract class BaseService implements IBaseService {
  ...
  protected async safeExecute<T>(
    ...
    try {
      logger.debug(`Starting operation: ${operationName} on service ${this.name}`);
      const result = await operation();
      logger.debug(`Completed operation: ${operationName} on service ${this.name}`);
      return result;
    } catch (error) {
      logger.error(`Operation failed: ${operationName} on service ${this.name}:`, error);
