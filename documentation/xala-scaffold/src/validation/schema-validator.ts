...
export const PREDEFINED_SCHEMAS = {
  ...
  xalaConfig: z.object({
    ...
    localization: z.object({
      ...
      supportedLocales: z.array(z.enum(['nb-NO', 'nn-NO', 'en-US', 'ar-SA', 'fr-FR'])),
...
export class SchemaValidator extends BaseValidator {
  ...
  async validateFile(file: string, content: string): Promise<ValidationIssue[]> {
    ...
    try {
      ...
      try {
        ...
        if (this.isJsonFile(file)) {
          issues.push({
            ...
            description: parseError instanceof Error ? parseError.message : 'JSON parsing failed',
            file,
      ...
      if (!result.success) {
        ...
        issues.push(...await this.validateSpecificFileType(file, fileType, data));
      }
  ...
  async validateData(data: any, schemaName: string): Promise<ValidationIssue[]> {
  ...
  private getFileType(file: string): string {
    ...
    if (fileName.includes('eslint') && fileName.endsWith('.json')) return 'eslintConfig';
  ...
  private getSchemaForFile(file: string, fileType: string): z.ZodSchema | undefined {
  ...
  private validateXalaConfig(file: string, data: any): ValidationIssue[] {
    ...
    if (data.compliance?.nsm?.classification === 'OPEN' && 
        data.compliance?.gdpr?.enabled === false) {
      issues.push({
        id: 'schema-xala-compliance-warning',
        severity: 'info',
        category: 'schema',
        message: 'Consider enabling GDPR compliance even for OPEN classification',
        description: 'GDPR compliance is recommended for all Norwegian projects',
        file,
        rule: 'schema.constraint-validation',
        fixable: false,
      });
