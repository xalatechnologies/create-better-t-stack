...
export abstract class BaseValidator {
	...
	async validate(
		...
		const fileMap = Array.isArray(files) ? await this.loadFiles(files) : files;
		...
		for (const [file, content] of fileMap) {
			...
			try {
				...
				if (
					...
					fileIssues.some((issue) => issue.severity === "error")
				) {
				...
				if (issues.length >= this.config.maxIssues) {
					logger.warn(
						`Reached maximum issues limit (${this.config.maxIssues})`,
					);
					break;
				}
			} catch (error) {
		...
		logger.info(
			`Validation completed: ${valid ? "PASSED" : "FAILED"} (${duration}ms)`,
		);
	...
	async autofix(result: ValidationResult): Promise<Map<string, string>> {
		...
		const fixableIssues = result.issues.filter(
			(issue) => issue.fixable && issue.autofix,
		);
	...
	protected calculateSummary(
		...
		for (const issue of issues) {
			...
			summary.categories[issue.category] =
				(summary.categories[issue.category] || 0) + 1;
		}
	...
	protected generateRecommendations(
		...
		if (result.summary.errorCount > 10) {
			recommendations.push({
				...
				description:
					"Your code has a high number of errors. Consider implementing stricter development practices.",
				benefits: [
				...
				effort: "medium",
				resources: ["ESLint configuration guide", "TypeScript best practices"],
			});
		}
		...
		if (result.summary.fixableCount > 5) {
			recommendations.push({
				...
				description:
					"Many issues can be automatically fixed. Consider enabling auto-fix in your workflow.",
				benefits: [
					"Reduced manual work",
				...
				effort: "low",
				resources: ["Auto-fix configuration", "CI/CD integration guide"],
			});
		}
		...
		if (result.score < 70) {
			recommendations.push({
				...
				description:
					"Your code quality score is below recommended standards. Consider refactoring.",
				benefits: [
					"Better code quality",
				...
				effort: "high",
				resources: ["Refactoring patterns", "Code review checklist"],
			});
		}
