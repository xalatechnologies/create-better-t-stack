...
export class SOLIDValidator extends BaseValidator {
	...
	private validateSRP(
		...
		if (analysis.exports.length > this.config.maxFileExports) {
			issues.push({
				...
				description:
					"Files with many exports may have multiple responsibilities",
				file,
		...
		analysis.classes.forEach((classAnalysis: ClassAnalysis) => {
			if (classAnalysis.methods.length > this.config.maxClassMethods) {
				issues.push({
					...
					description:
						"Classes with many methods may have multiple responsibilities",
					file,
		...
		analysis.classes.forEach((classAnalysis: ClassAnalysis) => {
			if (classAnalysis.extendsClass) {
				...
				issues.push({
					...
					description:
						"Ensure subclass is behaviorally substitutable for its superclass",
					file,
		...
		analysis.classes.forEach((classAnalysis: ClassAnalysis) => {
			...
			if (couplingScore > this.config.allowedCouplingScore) {
				issues.push({
					...
					description:
						"High coupling indicates tight dependencies; consider using abstractions",
					file,
	...
	private calculateInterfaceCohesion(
		methods: string[],
		properties: string[],
	): number {
	...
	private calculateCouplingScore(classAnalysis: ClassAnalysis): number {
		...
		return (
			classAnalysis.dependencies.length +
