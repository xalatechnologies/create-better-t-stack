...
export class AccessibilityValidator extends BaseValidator {
		...
		traverse(ast, {
			JSXElement: (path) => {
				...
				if (this.isInteractiveElement(elementName)) {
					issues.push(
						...this.validateInteractiveElement(file, element, elementName),
					);
				}
	...
	private validateStyling(
		...
		const colorRegex =
			/#[0-9a-fA-F]{3,8}|rgb\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*\)|rgba\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*[\d.]+\s*\)/g;
				...
				if (this.isReactComponent(path) && this.config.enforceFocusManagement) {
					...
					if (componentName && this.isModalOrDialogComponent(componentName)) {
						issues.push({
							...
							description:
								"Modal/dialog components should trap and restore focus",
							file,
		...
		if (content.includes("arabic") || content.includes("ar-SA")) {
			if (!content.includes("dir=") && !content.includes("direction")) {
				issues.push({
					...
					description:
						"Ensure proper RTL (right-to-left) text direction support",
					file,
