...
export class CodeStyleValidator extends BaseValidator {
  ...
  private validateFileStructure(file: string, content: string): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
      ...
      if (this.styleConfig.indentType === 'spaces' && hasTabs) {
        issues.push({
          ...
          autofix: {
            ...
            operation: 'replace',
            target: leadingWhitespace,
            replacement: leadingWhitespace.replace(/\t/g, ' '.repeat(this.styleConfig.indentSize)),
          },
        });
      }
      ...
      if (this.styleConfig.indentType === 'tabs' && hasSpaces) {
        issues.push({
          ...
          autofix: {
            ...
            operation: 'replace',
            target: leadingWhitespace,
            replacement: leadingWhitespace.replace(new RegExp(` {${this.styleConfig.indentSize}}`, 'g'), '\t'),
          },
        });
      }
    });
  ...
  private validateAST(file: string, content: string, ast: t.File): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    const lines = content.split('\n');
    ...
    traverse(ast, {
      ...
      VariableDeclarator(path) {
        ...
        if (t.isIdentifier(node.id)) {
          ...
          if (this.isReactComponent(path) && this.styleConfig.enforceComponentNaming) {
      ...
      TSInterfaceDeclaration(path) {
        ...
        if (node.id.name.endsWith('Props') && this.styleConfig.enforceReadonlyProps) {
            ...
            if (t.isTSPropertySignature(member) && !member.readonly) {
              issues.push({
                ...
                autofix: {
                  ...
                  operation: 'replace',
                  target: member.key && t.isIdentifier(member.key) ? member.key.name : '',
