...
export class SecurityValidator extends BaseValidator {
	...
	private initializeDangerousPatterns(): void {
		this.dangerousPatterns = [
			...
			{
				pattern:
					/localStorage\.setItem\s*\([^,]*,\s*.*(?:password|token|secret|key)/gi,
	...
	private validateSecretsExposure(
		file: string,
		content: string,
	): ValidationIssue[] {
		const issues: ValidationIssue[] = [];
	...
	private validateDangerousPatterns(
		file: string,
		content: string,
	): ValidationIssue[] {
		const issues: ValidationIssue[] = [];
		...
		this.dangerousPatterns.forEach((dangerousPattern) => {
			lines.forEach((line, index) => {
				if (dangerousPattern.pattern.test(line)) {
					const severity = this.mapSecuritySeverityToValidationSeverity(
						dangerousPattern.severity,
					);
			...
			JSXAttribute(path) {
				...
				if (
					...
					issues.push({
						...
						description:
							"Ensure HTML content is properly sanitized before using dangerouslySetInnerHTML",
						file,
						line: node.loc?.start.line,
	...
	private validateSQLInjection(file: string, ast: t.File): ValidationIssue[] {
		...
		traverse(ast, {
			...
			TemplateLiteral(path) {
				const node = path.node;
				const templateString = node.quasis
				...
				if (hasSQLKeywords && node.expressions.length > 0) {
					issues.push({
						...
						category: "security",
						message:
							"Potential SQL injection vulnerability in template literal",
						description:
							"SQL queries with user input should use parameterized queries",
						file,
						line: node.loc?.start.line,
			...
			BinaryExpression(path) {
				...
				if (node.operator === "+") {
					...
					if (leftString || rightString) {
						...
						const hasSQLKeywords = Array.from(this.sqlKeywords).some(
							(keyword) => upperString.includes(keyword),
						);
					...
					if (sensitiveOperations.includes(methodName.toLowerCase())) {
						...
						if (!hasAuthz) {
							issues.push({
								...
								description:
									"Sensitive operations should include authorization checks",
								file,
								line: node.loc?.start.line,
	...
	private validateDangerousFunctions(
		file: string,
		ast: t.File,
	): ValidationIssue[] {
		const issues: ValidationIssue[] = [];
			...
			StringLiteral(path) {
				...
				if (
					...
					issues.push({
						...
						description:
							"External URLs should use HTTPS for secure communication",
						file,
						line: node.loc?.start.line,
	...
	private validateNorwegianCompliance(
		...
		if (
			...
			issues.push({
				...
				description:
					"Security-related files should include NSM compliance information",
				file,
		...
		traverse(ast, {
			CallExpression(path) {
				...
				if (
					...
					if (
						["save", "create", "store", "log"].includes(
	...
	// Helper methods
	private mapSecuritySeverityToValidationSeverity(
		severity: SecuritySeverity,
	): ValidationSeverity {
		switch (severity) {
			case "critical":
