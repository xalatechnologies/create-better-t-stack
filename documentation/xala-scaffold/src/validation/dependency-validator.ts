...
export class DependencyValidator extends BaseValidator {
  ...
  private securityVulnerabilities = new Map<string, VulnerabilityInfo[]>([
    ...
    ['axios', [{
      ...
      description: 'Axios versions prior to 0.21.1 are vulnerable to prototype pollution',
  ...
  private deprecatedPackages = new Set([
    'request', 'bower', 'grunt', 'gulp-util', 'node-uuid'
  ]);
  ...
  constructor(config: Partial<DependencyConfig> = {}) {
    super({
      ...
      include: ['**/package.json', '**/*.ts', '**/*.tsx', '**/*.js', '**/*.jsx'],
      failFast: false,
      maxIssues: 1000,
    });
    ...
    this.config = {
      ...
      requiredPackages: ['@xala-technologies/ui-system', '@xala-technologies/foundation'],
  ...
  private async analyzePackageJson(file: string, content: string): Promise<PackageAnalysis> {
    ...
    await this.analyzeDependencySection(packageJson.dependencies, 'dependencies', analysis);
    await this.analyzeDependencySection(packageJson.devDependencies, 'devDependencies', analysis);
    await this.analyzeDependencySection(packageJson.peerDependencies, 'peerDependencies', analysis);
    
    // Find unused dependencies
    if (this.config.checkUnusedDependencies) {
      analysis.unusedDependencies = await this.findUnusedDependencies(file, analysis);
    }
  ...
  private validatePackageAnalysis(file: string, analysis: PackageAnalysis): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    ...
    if (analysis.outdatedDependencies.length > 0) {
      analysis.outdatedDependencies.forEach(dep => {
        issues.push({
          ...
          description: 'Consider updating to the latest version for security and performance improvements',
          file,
    ...
    if (analysis.unusedDependencies.length > 0) {
      analysis.unusedDependencies.forEach(dep => {
        issues.push({
          ...
          description: 'This dependency appears to be unused and can be removed',
          file,
    ...
    // Check bundle impact
    if (this.config.checkBundleImpact && analysis.bundleImpact > 1000) { // 1MB
      issues.push({
  ...
  private validateDependencyUsage(file: string, ast: t.File): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  private validateCircularDependencies(file: string, ast: t.File): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  private getDependencyInfo(analysis: PackageAnalysis, name: string): DependencyInfo | undefined {
    return analysis.dependencies.get(name) || 
  ...
  private hasDependency(analysis: PackageAnalysis, name: string): boolean {
    return analysis.dependencies.has(name) || 
  ...
  private async findUnusedDependencies(packageJsonPath: string, analysis: PackageAnalysis): Promise<string[]> {
    ...
    try {
          ...
          if (ast) {
            traverse(ast, {
              ImportDeclaration(path) {
                ...
                if (t.isStringLiteral(node.source)) {
                  const packageName = this.extractPackageName(node.source.value);
                  if (packageName) {
      ...
      for (const [name] of analysis.dependencies) {
        if (!usedPackages.has(name) && !this.config.requiredPackages.includes(name)) {
  ...
  private async getLicenseInfo(packageName: string, version: string): Promise<string | null> {
  ...
  private async isPackageOutdated(packageName: string, currentVersion: string): Promise<boolean> {
  ...
  private async estimateBundleSize(packageName: string, version: string): Promise<number | null> {
  ...
  private couldCreateCircularDependency(file: string, importPath: string): boolean {
