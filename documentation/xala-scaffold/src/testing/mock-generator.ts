...
export class MockGenerator {
   ...
   */
  generateNorwegianData<T = any>(type: keyof NorwegianDataTypes, count: number = 1): T[] {
...
export class NorwegianDataGenerator {
    ...
    for (let i = 0; i < count; i++) {
      switch (type) {
        ...
        case 'postalCode':
          ...
          break;
        case 'municipality':
          results.push(this.pickRandom(NORWEGIAN_MUNICIPALITIES) as NorwegianDataTypes[K]);
          break;
        case 'county':
          results.push(this.pickRandom(NORWEGIAN_COUNTIES) as NorwegianDataTypes[K]);
          break;
      }
    }
  ...
  generatePersonalData(options: MockGenerationOptions): any {
    const includePersonalId = options.includePersonalData && 
  ...
  generateAuditData(options: MockGenerationOptions): any {
    return {
      ...
      action: this.pickRandom(['create', 'read', 'update', 'delete', 'login', 'logout']),
  ...
  private generatePerson(): NorwegianPersonData {
    ...
    const firstNames = locale.startsWith('nb-') ? NORWEGIAN_FIRST_NAMES_NB : NORWEGIAN_FIRST_NAMES_NN;
  ...
  private generateCompany(): NorwegianCompanyData {
    const companyNames = [
      'Innovasjon AS', 'Teknologi Norge AS', 'Digital Løsninger AS',
      'Smart Systems AS', 'Fremtid Tech AS', 'Bærekraft Solutions AS',
    ];
    
    return {
      name: this.pickRandom(companyNames),
      organizationNumber: this.generateOrganizationNumber(),
      industry: this.pickRandom(['IT', 'Teknologi', 'Konsulentvirksomhet', 'Handel', 'Produksjon']),
  ...
  private generatePhoneNumber(): string {
    ...
    const number = prefix + Array.from({ length: 7 }, () => Math.floor(this.random() * 10)).join('');
  ...
  private generateIPAddress(): string {
    return Array.from({ length: 4 }, () => Math.floor(this.random() * 256)).join('.');
  }
...
export const ${serviceName}Mock = {
  ${methods.map(method => `
  ${method.name}: jest.fn()${method.async ? '.mockResolvedValue' : '.mockReturnValue'}(${this.generateMockReturnValue(method.returnType)}),`).join('')}
...
export const ${serviceName}TestHelpers = {
  ...
  mockSuccess: () => {
    ${methods.filter(m => m.async).map(method => `
    ${serviceName}Mock.${method.name}.mockResolvedValue(${this.generateMockReturnValue(method.returnType)});`).join('')}
  },
  
  mockError: (error = new Error('Test error')) => {
    ${methods.filter(m => m.async).map(method => `
    ${serviceName}Mock.${method.name}.mockRejectedValue(error);`).join('')}
  },
...
const NORWEGIAN_FIRST_NAMES_NB = [
  'Emma', 'Oliver', 'Nora', 'Jakob', 'Sofie', 'Lucas', 'Ella', 'William',
  'Leah', 'Filip', 'Sara', 'Noah', 'Ingrid', 'Emil', 'Maja', 'Mathias',
];

const NORWEGIAN_FIRST_NAMES_NN = [
  'Emma', 'Oliver', 'Nora', 'Jakob', 'Sofie', 'Lukas', 'Ella', 'William',
  'Lea', 'Filip', 'Sara', 'Noah', 'Ingrid', 'Emil', 'Maja', 'Mathias',
];
...
const NORWEGIAN_STREET_NAMES = [
  'Storgata', 'Kirkegata', 'Skolegata', 'Prestegata', 'Parkveien', 'Bjørkveien',
  'Granveien', 'Eikveien', 'Solveien', 'Haugeveien', 'Bakkeveien',
];

const NORWEGIAN_CITIES = [
  'Oslo', 'Bergen', 'Trondheim', 'Stavanger', 'Bærum', 'Kristiansand',
  'Fredrikstad', 'Tromsø', 'Sandnes', 'Drammen', 'Asker', 'Lillestrøm',
];
