...
import { TestFramework, TestType, NSMClassification } from './test-framework.js';
...
export class TestEnvironmentManager {
  ...
  private async setupComplianceTesting(classification: NSMClassification): Promise<void> {
  ...
  private async setupAccessibilityTesting(): Promise<void> {
    try {
      ...
      logger.warn('jest-axe not available, accessibility testing will be limited');
    }
  }
...
export class ComplianceAssertions {
  ...
  static assertGDPRCompliance(element: Element): void {
    ...
    for (const attr of dataAttributes) {
      ...
      expect(attr.value).not.toMatch(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/); // Email
    }
  }
   ...
   */
  static assertNSMCompliance(element: Element, classification: NSMClassification): void {
    ...
    if (element.tagName.toLowerCase() === 'form') {
      ...
      if (classification !== NSMClassification.OPEN) {
        ...
        const csrfToken = element.querySelector('input[name="_token"]') ||
...
export class NorwegianLocalizationTestingUtils {
   ...
   */
  static assertNorwegianText(element: Element, expectedText: string, locale: LocaleCode): void {
   ...
   */
  static assertNorwegianDateFormat(dateString: string, locale: LocaleCode): void {
   ...
   */
  static assertNorwegianNumberFormat(numberString: string, value: number, locale: LocaleCode): void {
 ...
 */
export async function setupTestEnvironment(overrides?: Partial<TestEnvironmentConfig>): Promise<void> {
