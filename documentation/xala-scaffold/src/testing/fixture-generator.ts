...
import { NorwegianDataGenerator, NorwegianPersonData, NorwegianAddressData } from './mock-generator.js';
...
export class FixtureGenerator {
  ...
  constructor(private defaultOptions: FixtureGenerationOptions = {}) {
    ...
    this.componentGenerator = new ComponentFixtureGenerator(this.dataGenerator, defaultOptions);
    this.apiGenerator = new APIFixtureGenerator(this.dataGenerator, defaultOptions);
    this.databaseGenerator = new DatabaseFixtureGenerator(this.dataGenerator, defaultOptions);
    this.localizationGenerator = new LocalizationFixtureGenerator(this.dataGenerator, defaultOptions);
    this.complianceGenerator = new ComplianceFixtureGenerator(this.dataGenerator, defaultOptions);
  }
  ...
  private generatePerformanceData(options: FixtureGenerationOptions): {
    ...
    const fixtures = {
      ...
      hugeLargeArray: new Array(10000).fill(null).map((_, i) => ({ id: i, value: `item-${i}` })),
...
`;
  ...
  private generateAccessibilityData(): {
    ...
    const fixtures = {
      ...
      focusStates: {
        valid: [
          { element: 'button', focusVisible: true, outline: '2px solid #007acc' },
          { element: 'input', focusVisible: true, outline: '2px solid #007acc' },
        ],
        invalid: [
...
export class ComponentFixtureGenerator {
  ...
  generate(options: ComponentFixtureOptions): {
    ...
    const fixtures = {
      ...
      edgeCases: options.includeEdgeCases ? this.generateEdgeCases(componentName, options) : {},
      errorStates: options.includeErrorStates ? this.generateErrorStates(componentName, options) : {},
      compliance: this.generateComplianceVariants(componentName, options),
    };
    
    const stories = options.includeStorybook ? this.generateStorybookStories(componentName, fixtures) : undefined;
  ...
  private generateDefaultProps(componentName: string, options: ComponentFixtureOptions): any {
    ...
    if (options.classification && options.classification !== NSMClassification.OPEN) {
      return {
        ...commonProps,
  ...
  private generateVariants(componentName: string, options: ComponentFixtureOptions, count: number): any[] {
    const variants = [];
  ...
  private generateEdgeCases(componentName: string, options: ComponentFixtureOptions): any {
    return {
      emptyProps: {},
      nullValues: {
        title: null,
  ...
  private generateErrorStates(componentName: string, options: ComponentFixtureOptions): any {
    return {
      loadingState: {
        ...this.generateDefaultProps(componentName, options),
        loading: true,
  ...
  private generateComplianceVariants(componentName: string, options: ComponentFixtureOptions): any {
    const classifications = [NSMClassification.OPEN, NSMClassification.RESTRICTED, NSMClassification.CONFIDENTIAL];
    ...
    for (const classification of classifications) {
      for (const locale of locales) {
        ...
        variants[key] = {
          ...this.generateDefaultProps(componentName, { ...options, classification, locale }),
          classification,
          locale,
  ...
  private generateStorybookStories(componentName: string, fixtures: Record<string, any>): string {
    return `
...
// Variants
${fixtures.variants.map((variant: any, index: number) => `
...
`;
  ...
  private generateTypeDefinitions(componentName: string, fixtures: Record<string, any>): string {
    return `
...
export class APIFixtureGenerator {
  ...
  generate(options: APIFixtureOptions): {
    ...
    const fixtures = {
      ...
      pagination: options.includePagination ? this.generatePaginationResponses(options) : {},
  ...
  private generateSuccessResponses(options: APIFixtureOptions): any {
    ...
    switch (options.method) {
      ...
      case 'DELETE':
        return {
          deleted: { message: 'Resource deleted successfully', deletedAt: new Date().toISOString() },
        };
  ...
  private generateLoadingStates(options: APIFixtureOptions): any {
    return {
      ...
      skeleton: { loading: true, data: this.generateSkeletonData(options), error: null },
    };
  }
  ...
  private generatePaginationResponses(options: APIFixtureOptions): any {
    ...
    return {
      ...
      lastPage: {
        data: this.generateResourceList(options, totalItems % pageSize || pageSize),
        meta: {
          total: totalItems,
          page: totalPages,
          limit: pageSize,
  ...
  private generateResourceList(options: APIFixtureOptions, count: number): any[] {
    const resources = [];
    for (let i = 0; i < count; i++) {
      resources.push({
        ...this.generateSingleResource(options),
  ...
  private generateMSWHandlers(options: APIFixtureOptions, fixtures: any): string {
...
export class DatabaseFixtureGenerator {
  ...
  private generateTableData(options: DatabaseFixtureOptions, count: number): any[] {
    const data: any[] = [];
  ...
  private generateRelatedTableData(
    ...
    for (const parent of parentData) {
      const relatedCount = relationship.relationType === 'one-to-one' ? 1 : 
                          relationship.relationType === 'one-to-many' ? Math.floor(Math.random() * 5) + 1 : 
    ...
    switch (type.toLowerCase()) {
      ...
      case 'text':
        if (field.includes('name')) {
          const person = this.dataGenerator.generate('person', 1)[0];
          return field.includes('first') ? person.firstName : 
                 field.includes('last') ? person.lastName : person.fullName;
        }
  ...
  private generateSeedFile(options: DatabaseFixtureOptions, fixtures: Record<string, any[]>): string {
    return `
-- Database seeds for ${options.tableName}
-- Auto-generated test fixtures with Norwegian compliance

${Object.entries(fixtures).map(([tableName, data]) => `
-- Seeds for ${tableName}
INSERT INTO ${tableName} (${Object.keys(data[0] || {}).join(', ')})
VALUES
${data.map(record => `(${Object.values(record).map(value => 
...
CREATE TABLE ${options.tableName} (
  ${Object.entries(options.schema).map(([field, type]) => {
...
`;
  ...
  private mapTypeToSQL(type: string): string {
    switch (type.toLowerCase()) {
      case 'id':
      case 'uuid': return 'UUID DEFAULT gen_random_uuid()';
      case 'string':
...
export class LocalizationFixtureGenerator {
  ...
  private generateLocaleFixtures(locale: LocaleCode): Record<string, string> {
    ...
    const commonKeys = [
      'welcome', 'hello', 'goodbye', 'yes', 'no', 'ok', 'cancel', 'save', 'delete',
      'edit', 'create', 'update', 'submit', 'reset', 'close', 'open', 'back', 'next',
      'previous', 'home', 'about', 'contact', 'help', 'settings', 'profile', 'logout',
      'loading', 'error', 'success', 'warning', 'info', 'required', 'optional'
    ];
    ...
    translations['compliance.nsm.classification'] = this.getTranslation('nsm_classification', locale);
    translations['compliance.gdpr.consent'] = this.getTranslation('gdpr_consent', locale);
    translations['compliance.wcag.accessibility'] = this.getTranslation('accessibility', locale);
    
    // Form validation messages
    translations['validation.required'] = this.getTranslation('field_required', locale);
    translations['validation.email'] = this.getTranslation('invalid_email', locale);
    translations['validation.phone'] = this.getTranslation('invalid_phone', locale);
  ...
  private generateLocalizationTestHelpers(fixtures: Record<LocaleCode, Record<string, string>>): string {
    return `
...
export class ComplianceFixtureGenerator {
  ...
  private generateNSMScenarios(classification: NSMClassification): any {
    return {
      ...
      [NSMClassification.CONFIDENTIAL]: {
        ...
        requiresAuditLogging: true,
        allowedLocations: ['NO'],
        securityHeaders: ['X-Frame-Options', 'X-Content-Type-Options', 'Strict-Transport-Security', 'Content-Security-Policy'],
      },
      [NSMClassification.SECRET]: {
        ...
        requiresAuditLogging: true,
        allowedLocations: ['NO'],
        securityHeaders: ['X-Frame-Options', 'X-Content-Type-Options', 'Strict-Transport-Security', 'Content-Security-Policy', 'Referrer-Policy'],
      },
    };
  }
  ...
  private generateGDPRScenarios(classification: NSMClassification): any {
    return {
      ...
      privacyByDesign: true,
      dpoRequired: classification === NSMClassification.CONFIDENTIAL || classification === NSMClassification.SECRET,
      allowedCountries: classification === NSMClassification.SECRET ? ['NO'] : ['NO', 'EU'],
    };
  }
  ...
  private generateWCAGScenarios(): any {
    return {
      ...
      testCases: [
        { element: 'img', requirement: 'alt', valid: 'Image description', invalid: '' },
        { element: 'button', requirement: 'aria-label', valid: 'Submit form', invalid: null },
        { element: 'input', requirement: 'label', valid: 'associated', invalid: 'missing' },
      ],
    };
  }
