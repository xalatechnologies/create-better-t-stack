...
export class BoltAdapter extends BaseMigrationAdapter {
  ...
  async analyzeProject(projectPath: string): Promise<ProjectAnalysis> {
    ...
    // Analyze components
    analysis.components = await this.analyzeComponents(projectPath, analysis.structure);
    ...
    // Analyze routing
    analysis.routing = await this.analyzeRouting(projectPath, analysis.framework);
    ...
    logger.info(`Bolt.new analysis complete: ${analysis.components.length} components, ${analysis.pages.length} routes`);
    
    return analysis;
  }
  
  // Transform Bolt component to Xala component
  async transformComponent(component: ComponentInfo, analysis: ProjectAnalysis): Promise<string> {
    ...
    try {
      ...
      traverse(ast, {
        ...
        ImportDeclaration(nodePath) {
          ...
          } else if (BOLT_PATTERNS.boltDependencies.some(dep => source.includes(dep))) {
            // Transform Bolt imports to Xala imports
            this.transformBoltImports(nodePath, imports);
          } else {
            // Keep other imports
            const importCode = sourceCode.substring(nodePath.node.start!, nodePath.node.end!);
      ...
      logger.error(`Error transforming Bolt component ${component.name}:`, error);
      throw error;
    }
  }
  
  // Transform Bolt route/page to Xala page
  async transformPage(page: PageInfo, analysis: ProjectAnalysis): Promise<string> {
  ...
  async transformStyles(styling: StylingInfo, analysis: ProjectAnalysis): Promise<Record<string, string>> {
    ...
    for (const apiDir of apiDirs) {
      ...
      if (await fileExists(fullPath)) {
        try {
          ...
          if (files.some(file => file.toString().endsWith('.ts') || file.toString().endsWith('.tsx'))) {
            return true;
          }
        } catch (error) {
          continue;
  ...
  private async analyzeStructure(projectPath: string): Promise<ProjectStructure> {
    ...
    const commonDirs = ['app', 'app/components', 'app/routes', 'public', 'prisma'];
  ...
  private async analyzeComponents(projectPath: string, structure: ProjectStructure): Promise<ComponentInfo[]> {
  ...
  private async analyzeRoutes(projectPath: string, structure: ProjectStructure): Promise<PageInfo[]> {
  ...
  private async analyzeAssets(projectPath: string, structure: ProjectStructure): Promise<any[]> {
  ...
  private async analyzeRouting(projectPath: string, framework: string): Promise<any> {
    if (framework === 'remix') {
      return {
        type: 'remix-router',
  ...
  private async analyzeComponent(filePath: string, projectPath: string): Promise<ComponentInfo | null> {
  ...
  private async analyzeRoute(filePath: string, projectPath: string): Promise<PageInfo | null> {
    const sourceCode = await readFile(filePath);
    const relativePath = path.relative(projectPath, filePath);
    ...
    if (sourceCode.includes('requireAuth') || sourceCode.includes('authenticate')) {
      info.hasAuth = true;
    }
  ...
  private transformBoltImports(nodePath: any, imports: string[]): void {
    ...
    if (xalaImports.length > 0) {
      imports.push(`import { ${xalaImports.join(', ')} } from '@xala-technologies/ui-system';`);
    }
  }
  ...
  private transformBoltJSXElement(nodePath: any): void {
    ...
    if (t.isJSXIdentifier(elementName)) {
      ...
      if (mappedName) {
        ...
        if (nodePath.node.closingElement?.name && t.isJSXIdentifier(nodePath.node.closingElement.name)) {
...
`
  ...
  private generateXalaComponent(config: {
    ...
    const { name, imports, hasRemixFeatures, typescript, styling, localization } = config;
    ...
    // Add component
    const propsParam = typescript ? `{ className, ${localization ? 'ariaLabel' : ''} }: ${name}Props` : 'props';
  ...
  private generateXalaPage(config: {
    ...
    const { name, route, hasAuth, hasLayout, seo, platform, typescript, localization } = config;
...
[dir="rtl"] .xala-input {
  ...
  protected async migrateComponentTests(component: ComponentInfo, result: MigrationResult): Promise<void> {
  ...
  protected async migrateComponentStories(component: ComponentInfo, result: MigrationResult): Promise<void> {
  ...
  protected async generateConfigurationFiles(result: MigrationResult): Promise<void> {
    const configs = {
      'tsconfig.json': this.generateTsConfig(),
      'tailwind.config.js': this.generateTailwindConfig(),
      'next.config.js': this.options.targetPlatform === 'nextjs' ? this.generateNextConfig() : null,
      'vite.config.ts': this.options.targetPlatform === 'react' ? this.generateViteConfig() : null,
    };
  ...
  protected async generatePackageJson(analysis: ProjectAnalysis): Promise<void> {
    const packageJson = {
      ...
      scripts: {
        ...
        build: this.options.targetPlatform === 'nextjs' ? 'next build' : 'vite build',
        start: this.options.targetPlatform === 'nextjs' ? 'next start' : 'vite preview',
