...
export abstract class BaseMigrationAdapter {
	...
	abstract transformPage(
		page: PageInfo,
		analysis: ProjectAnalysis,
	): Promise<string>;
	abstract transformStyles(
		styling: StylingInfo,
		analysis: ProjectAnalysis,
	...
	async migrate(
		...
		try {
			...
			if (!(await this.canMigrate(sourcePath))) {
				throw new Error(
					"Source project is not compatible with this migration adapter",
				);
			}
	...
	protected async performMigration(result: MigrationResult): Promise<void> {
		...
		for (const component of result.analysis.components) {
			try {
				const transformedContent = await this.transformComponent(
					component,
					result.analysis,
				);
		...
		for (const page of result.analysis.pages) {
			try {
				const transformedContent = await this.transformPage(
					page,
					result.analysis,
				);
				...
				result.errors.push({
					...
					type: "transform",
					message:
						error instanceof Error ? error.message : "Page migration failed",
					originalError: error instanceof Error ? error : undefined,
				});
			}
		}
		...
		// Migrate styles
		try {
			const transformedStyles = await this.transformStyles(
			...
			result.errors.push({
				...
				type: "transform",
				message:
					error instanceof Error ? error.message : "Style migration failed",
				originalError: error instanceof Error ? error : undefined,
			});
		}
	...
	protected abstract migrateComponentTests(
		component: ComponentInfo,
		result: MigrationResult,
	): Promise<void>;
	protected abstract migrateComponentStories(
		component: ComponentInfo,
		result: MigrationResult,
	): Promise<void>;
	protected abstract generateConfigurationFiles(
		result: MigrationResult,
	): Promise<void>;
	protected abstract generatePackageJson(
		analysis: ProjectAnalysis,
	): Promise<void>;
...
`;
	...
	protected async generateLocalizationFiles(
		texts: ExtractedText[],
	): Promise<void> {
