...
export class LovableAdapter extends BaseMigrationAdapter {
  ...
  async canMigrate(projectPath: string): Promise<boolean> {
    try {
      ...
      if (await fileExists(srcPath)) {
        const hasLovableComponents = await this.checkForLovableComponents(srcPath);
  ...
  async analyzeProject(projectPath: string): Promise<ProjectAnalysis> {
    ...
    logger.info(`Analysis complete: ${analysis.components.length} components, ${analysis.pages.length} pages`);
    ...
    try {
      ...
      traverse(ast, {
        ...
        ImportDeclaration(nodePath) {
          ...
          if (LOVABLE_PATTERNS.lovableImports.some(pattern => source.includes(pattern))) {
            ...
            const specifiers = nodePath.node.specifiers
              .map(spec => {
                if (t.isImportSpecifier(spec) && t.isIdentifier(spec.imported)) {
                  ...
                  const mappedName = COMPONENT_MAPPING[importedName] || importedName.replace(/^Lovable/, '');
                  return mappedName;
                }
                return null;
              })
            ...
            if (specifiers.length > 0) {
              imports.push(`import { ${specifiers.join(', ')} } from '@xala-technologies/ui-system';`);
            }
        ...
        FunctionDeclaration(nodePath) {
          if (t.isIdentifier(nodePath.node.id) && nodePath.node.id.name === componentName) {
  ...
  private async analyzeStructure(projectPath: string): Promise<ProjectStructure> {
    ...
    const commonDirs = ['src', 'components', 'pages', 'styles', 'assets', 'public'];
  ...
  private async analyzePages(projectPath: string, structure: ProjectStructure): Promise<PageInfo[]> {
  ...
  private async analyzePage(filePath: string, projectPath: string): Promise<PageInfo | null> {
    const sourceCode = await readFile(filePath);
    const relativePath = path.relative(projectPath, filePath);
  ...
  private generateXalaComponent(config: {
    ...
    const { name, props, imports, hasState, hooks, styling, typescript, localization } = config;
    ...
    const propsParam = props.length > 0 ? 
      (typescript ? `{ ${props.map(p => p.name).join(', ')}, className, ${localization ? 'ariaLabel' : ''} }: ${name}Props` : 'props') : 
      'props';
