...
import {
	EmbeddingGenerator,
	IEmbeddingOptions,
} from "../vector-store/EmbeddingGenerator.js";
...
export class CodeRetriever extends EventEmitter {
	...
	async searchCode(
		...
		try {
			...
			for (const collectionName of collections) {
				try {
					const collection =
						await this.vectorStore.getCollection(collectionName);
					const results = await collection.query([embeddingResult.embedding], {
	...
	async findSimilarPatterns(
		...
		try {
			...
			const embeddingResult =
				await this.embeddingGenerator.generateCodeEmbedding(codeContent, {
			...
			this.logger.debug("Similar patterns found", {
				...
				avgSimilarity:
					similarPatterns.length > 0
						? similarPatterns.reduce((sum, p) => sum + p.score, 0) /
	...
	async getRecommendations(
		...
		try {
			...
			if (context.project_type) {
				const projectRecommendations = await this.getProjectTypeRecommendations(
			...
			if (context.locale) {
				const localeRecommendations = await this.getLocaleRecommendations(
					context.locale,
				);
	...
	private async enhanceQuery(
		...
		const complianceKeywords = this.getComplianceKeywords(
			query.compliance,
			locale,
		);
	...
	private getComplianceKeywords(
		compliance?: NorwegianCompliance,
		locale: LocaleCode = "nb-NO",
	): string[] {
		const keywords: string[] = [];
		const localeKeywords =
			this.NORWEGIAN_KEYWORDS[locale] || this.NORWEGIAN_KEYWORDS["en-US"];
			...
			if (compliance.wcagLevel === "AAA") {
				keywords.push(
					...localeKeywords.filter(
						(k) =>
							k.includes("wcag") ||
							k.includes("tilgjengelighet") ||
							k.includes("accessibility"),
					),
				);
			}
		}
	...
	private async scoreAndRankResults(
		...
		for (const result of results) {
			...
			// Type match score
			const typeMatchScore = this.calculateTypeMatchScore(
				query,
				result.metadata,
			);
			...
			scoredResults.push({
				...
				relevance_explanation: this.generateRelevanceExplanation(
					result,
					query,
					finalScore,
				),
	...
	private calculateComplianceScore(
		...
		if (
			queryCompliance.nsmClassification ===
			metadata.compliance.nsmClassification
		) {
			score += 0.4;
		}
	...
	private async getProjectTypeRecommendations(
		projectType: string,
	): Promise<ICodeSearchResult[]> {
	...
	private async getLocaleRecommendations(
		locale: LocaleCode,
	): Promise<ICodeSearchResult[]> {
		const query: ICodeQuery = {
	...
	private updateAnalytics(
		...
		this.analyticsData.avg_response_time =
			(this.analyticsData.avg_response_time *
				(this.analyticsData.total_queries - 1) +
				processingTime) /
			this.analyticsData.total_queries;
