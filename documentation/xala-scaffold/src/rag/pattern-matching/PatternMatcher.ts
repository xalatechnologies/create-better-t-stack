		...
		try {
			...
			for (const pattern of this.knownPatterns.values()) {
				...
				if (options.focus_areas && options.focus_areas.length > 0) {
					const hasMatchingArea = options.focus_areas.some(
						(area) =>
			...
			this.logger.debug("Pattern analysis completed", {
				...
				avgScore:
					sortedMatches.length > 0
						? sortedMatches.reduce((sum, m) => sum + m.match_score, 0) /
			...
			for (const pattern of this.knownPatterns.values()) {
				...
				if (similarity >= minSimilarity) {
					matches.push({
						...
						matched_segments: await this.findMatchedSegments(
							code,
							pattern.pattern_code,
						),
						improvements: [],
  ...
  async createUser(userData: CreateUserRequest): Promise<User> {
	...
	private async matchPattern(
		...
		try {
			...
			const structuralSimilarity = ast
				? await this.calculateStructuralSimilarity(ast, pattern)
				: 0;
			...
			const matchedSegments = await this.findMatchedSegments(
				code,
				pattern.pattern_code,
			);
	...
	private async findMatchedSegments(
		code: string,
		pattern: string,
	): Promise<IMatchedSegment[]> {
		...
		if (pattern.compliance) {
			if (!code.includes("nsmClassification")) {
				improvements.push({
					...
					reasoning:
						"Norwegian compliance requires NSM classification for all sensitive data",
			...
			if (!code.includes("gdprCompliant")) {
				improvements.push({
					...
					reasoning:
						"GDPR compliance must be explicitly declared for data processing",
			...
			if (
				...
				if (line.includes("=") && !line.includes("process.env")) {
					issues.push({
						...
						documentation_url:
							"https://nsm.no/regelverk/grunnprinsipper-for-ikt-sikkerhet",
					});
				}
			}
		}
	...
	private generateSimilarityExplanation(
		...
		if (similarity > 0.9)
			return `Very high similarity (${(similarity * 100).toFixed(1)}%) - nearly identical to ${pattern.name}`;
		if (similarity > 0.8)
			return `High similarity (${(similarity * 100).toFixed(1)}%) - closely matches ${pattern.name}`;
		if (similarity > 0.7)
			return `Good similarity (${(similarity * 100).toFixed(1)}%) - similar approach to ${pattern.name}`;
