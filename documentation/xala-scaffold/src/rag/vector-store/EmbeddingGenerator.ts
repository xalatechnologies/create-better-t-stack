...
export interface IEmbeddingProvider {
  generateEmbedding(text: string, options?: IEmbeddingOptions): Promise<IEmbeddingResult>;
  generateBatchEmbeddings(texts: string[], options?: IEmbeddingOptions): Promise<IBatchEmbeddingResult>;
...
export class SentenceTransformersProvider implements IEmbeddingProvider {
  ...
  constructor(
    ...
    this.model = this.config.get<string>('ai.embeddings.model', 'all-MiniLM-L6-v2');
    this.dimensions = this.config.get<number>('ai.embeddings.dimensions', 384);
    this.apiUrl = this.config.get<string>('ai.embeddings.apiUrl', 'http://localhost:8001');
  }

  async generateEmbedding(text: string, options: IEmbeddingOptions = {}): Promise<IEmbeddingResult> {
  ...
  async generateBatchEmbeddings(texts: string[], options: IEmbeddingOptions = {}): Promise<IBatchEmbeddingResult> {
    const startTime = Date.now();
    ...
    try {
      this.logger.debug('Generating batch embeddings', {
        model: this.model,
        count: texts.length,
        avgLength: texts.reduce((sum, text) => sum + text.length, 0) / texts.length
      });
      ...
      for (let i = 0; i < texts.length; i += batchSize) {
        ...
        try {
          ...
          const batchEmbeddings = batch.map(() => this.createNormalizedRandomEmbedding(this.dimensions));
...
export class OpenAICompatibleProvider implements IEmbeddingProvider {
  ...
  constructor(
    ...
    this.model = this.config.get<string>('ai.embeddings.openaiModel', 'text-embedding-ada-002');
    this.dimensions = this.config.get<number>('ai.embeddings.openaiDimensions', 1536);
    this.apiUrl = this.config.get<string>('ai.embeddings.openaiUrl', 'http://localhost:8002');
  ...
  async generateEmbedding(text: string, options: IEmbeddingOptions = {}): Promise<IEmbeddingResult> {
    ...
    try {
      ...
      this.logger.error('Failed to generate OpenAI-compatible embedding', error as Error);
  ...
  async generateBatchEmbeddings(texts: string[], options: IEmbeddingOptions = {}): Promise<IBatchEmbeddingResult> {
    ...
    try {
      ...
      this.logger.error('Failed to generate OpenAI-compatible batch embeddings', error as Error);
      
      // Fallback to random embeddings
      return {
        embeddings: texts.map(() => this.createNormalizedRandomEmbedding(this.dimensions)),
...
export class EmbeddingGenerator extends EventEmitter {
  ...
  constructor(
    ...
    this.maxCacheSize = this.config.get<number>('ai.embeddings.cacheSize', 10000);
    this.defaultProvider = this.config.get<string>('ai.embeddings.defaultProvider', 'sentence-transformers');
  ...
  async generateCodeEmbedding(
    ...
    try {
      ...
      const result = await this.generateEmbedding(processedCode, embeddingOptions);
  ...
  async generateBatchEmbeddings(
    ...
    try {
      ...
      const { cachedResults, uncachedTexts, uncachedIndices } = this.checkBatchCache(texts, options);
      ...
      const batchResult = await provider.generateBatchEmbeddings(uncachedTexts, options);
      ...
      const result: IBatchEmbeddingResult = {
        ...
        model: batchResult.model,
        dimensions: batchResult.dimensions,
        failed_indices: batchResult.failed_indices.map(idx => uncachedIndices[idx])
      };
  ...
  async generateChunkEmbeddings(
    ...
    try {
      this.logger.debug('Generating chunk embeddings', { chunksCount: chunks.length });
  ...
  private initializeProviders(): void {
    ...
    this.providers.set('sentence-transformers', 
