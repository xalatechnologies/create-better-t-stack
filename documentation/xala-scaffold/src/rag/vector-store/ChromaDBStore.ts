...
export class ChromaDBCollection {
  ...
  async add(documents: IVectorDocument[]): Promise<void> {
    try {
      ...
      this.logger.error('Failed to add documents to collection', error as Error, {
        collection: this.name,
        count: documents.length
      });
      throw error;
  ...
  async update(documents: IVectorDocument[]): Promise<void> {
    try {
      ...
      this.logger.error('Failed to update documents in collection', error as Error, {
        collection: this.name,
        count: documents.length
      });
      throw error;
  ...
  async delete(ids: string[]): Promise<void> {
    try {
      ...
      this.logger.error('Failed to delete documents from collection', error as Error, {
        collection: this.name,
        count: ids.length
      });
      throw error;
        ...
        for (let i = 0; i < response.documents[0].length; i++) {
          ...
          if (score >= threshold) {
            results.push({
              ...
              metadata: include_metadata ? response.metadatas[0][i] : {} as IVectorDocumentMetadata,
              embedding: include_embeddings ? response.embeddings[0][i] : undefined,
              score,
              distance
            });
          }
      ...
      if (response.documents) {
        for (let i = 0; i < response.documents.length; i++) {
          documents.push({
            ...
            metadata: response.metadatas ? response.metadatas[i] : {} as IVectorDocumentMetadata,
  ...
  async count(): Promise<number> {
    try {
      ...
      this.logger.error('Failed to count documents in collection', error as Error, {
        collection: this.name
      });
      throw error;
    }
...
export class ChromaDBStore extends EventEmitter {
   ...
   */
  async getCollection(name: string, metadata?: any): Promise<ChromaDBCollection> {
    ...
    const collection = new ChromaDBCollection(name, this.chromaClient, this.logger);
  ...
  async deleteCollection(name: string): Promise<void> {
    try {
      ...
      this.logger.error('Failed to delete collection', error as Error, { name });
      throw error;
    }
  }
  ...
  async getOverallStats(): Promise<{
    ...
    try {
      ...
      return {
        ...
        avg_health_score: collectionNames.length > 0 ? totalHealthScore / collectionNames.length : 0
      };
    } catch (error) {
  ...
  private createChromaClient(): IChromaClient {
    const baseUrl = this.config.get<string>('ai.chroma.url', 'http://localhost:8000');
    ...
    return {
      ...
      async addDocuments(collection: string, data: any) {
        const response = await fetch(`${baseUrl}/api/v1/collections/${collection}/add`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      async updateDocuments(collection: string, data: any) {
        const response = await fetch(`${baseUrl}/api/v1/collections/${collection}/update`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(data)
        });
      ...
      async deleteDocuments(collection: string, data: any) {
        const response = await fetch(`${baseUrl}/api/v1/collections/${collection}/delete`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(data)
        });
      ...
      async query(collection: string, params: any) {
        const response = await fetch(`${baseUrl}/api/v1/collections/${collection}/query`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      async get(collection: string, params: any) {
        const response = await fetch(`${baseUrl}/api/v1/collections/${collection}/get`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(params)
        });
      ...
      async count(collection: string) {
        const response = await fetch(`${baseUrl}/api/v1/collections/${collection}/count`);
  ...
  private async initializeDefaultCollections(): Promise<void> {
    ...
    for (const collectionConfig of defaultCollections) {
      try {
        await this.getCollection(collectionConfig.name, collectionConfig.metadata);
      } catch (error) {
