...
import { ChromaDBStore, IVectorDocument } from '../vector-store/ChromaDBStore.js';
import { EmbeddingGenerator } from '../vector-store/EmbeddingGenerator.js';
import { PatternMatcher, ICodePattern } from '../pattern-matching/PatternMatcher.js';
  ...
  private readonly BUILT_IN_PATTERNS: ICodePattern[] = [
    {
      ...
      description: 'Standard interface for Norwegian compliance requirements including NSM, GDPR, and WCAG',
      type: 'compliance',
      language: 'typescript',
      pattern_code: `
...
interface LocaleCode {
  ...
  auditTrail: true
      ...
      related_patterns: ['component-with-compliance', 'service-with-compliance'],
      compliance: {
    ...
    },
    {
      ...
      description: 'WCAG AAA compliant button component with Norwegian localization support',
      type: 'structural',
      language: 'typescript',
      framework: 'react',
      pattern_code: `
...
export const AccessibleButton = ({
  ...
  return (
    ...
    >
      ...
      metadata: {
        category: 'component',
        tags: ['button', 'accessible', 'wcag', 'react', 'typescript', 'norwegian'],
        quality_score: 0.92,
        usage_count: 0,
        complexity: 4,
...
>
  Send inn
      ...
      related_patterns: ['accessible-form-component', 'norwegian-compliance-interface'],
      compliance: {
        nsmClassification: 'OPEN',
        gdprCompliant: true,
        wcagLevel: 'AAA',
    ...
    },
    {
      ...
      description: 'Form component with GDPR compliance, consent management, and Norwegian localization',
      type: 'structural',
      language: 'typescript',
      framework: 'react',
      pattern_code: `
...
export const GDPRCompliantForm = ({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      metadata: {
        category: 'component',
        tags: ['form', 'gdpr', 'consent', 'privacy', 'norwegian', 'typescript', 'react'],
        quality_score: 0.94,
        usage_count: 0,
        complexity: 6,
...
>
  ...
  <input name="name" type="text" required />
    ...
    },
    {
      ...
      description: 'Norwegian compliance audit logging service with NSM classification support',
      type: 'structural',
      language: 'typescript',
      pattern_code: `
...
export const auditLogger = new AuditLoggerService(
  /* inject dependencies */
      ...
      metadata: {
        category: 'service',
        tags: ['audit', 'logging', 'nsm', 'gdpr', 'security', 'norwegian', 'typescript'],
        quality_score: 0.91,
        usage_count: 0,
        complexity: 8,
...
auditLogger.logGDPRConsent({
  ...
  dataSubject: 'user@example.com'
      ...
      related_patterns: ['gdpr-compliant-form', 'norwegian-compliance-interface'],
      compliance: {
        nsmClassification: 'RESTRICTED',
        gdprCompliant: true,
        wcagLevel: 'A',
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.error('Failed to initialize Knowledge Base Seeder', error as Error);
      throw error;
    }
  }
   ...
   */
  async seedKnowledgeBase(options: ISeedingOptions = {}): Promise<ISeedingResult> {
    ...
    try {
      ...
      if (includeBuiltInPatterns) {
        const builtInResult = await this.seedBuiltInPatterns(generateEmbeddings, validatePatterns, override);
      ...
      if (includeComplianceRules) {
        const complianceResult = await this.seedComplianceRules(locale, generateEmbeddings, override);
      ...
      // Seed examples
      if (includeExamples) {
        const examplesResult = await this.seedExamples(generateEmbeddings, validatePatterns, override);
  ...
  ): Promise<{ patterns: number; examples: number; errors: string[]; warnings: string[] }> {
    try {
      ...
      for (const file of codeFiles) {
        try {
          ...
          if (isPattern) {
            await this.addPatternFromFile(file, content, generateEmbeddings, validatePatterns, override);
            result.patterns++;
          } else {
            await this.addExampleFromFile(file, content, generateEmbeddings, override);
            result.examples++;
          }

        } catch (fileError) {
          result.errors.push(`Failed to process file ${file}: ${fileError instanceof Error ? fileError.message : 'Unknown error'}`);
        }
      }
      ...
      this.logger.error('Failed to seed from directory', error as Error, { path: directoryPath });
      throw error;
    }
  }
  ...
  private async seedBuiltInPatterns(
    ...
    this.logger.debug('Seeding built-in patterns', { count: this.BUILT_IN_PATTERNS.length });
    ...
    for (const pattern of this.BUILT_IN_PATTERNS) {
      try {
        ...
        const vectorDoc = await this.convertPatternToVectorDocument(pattern, generateEmbeddings);
        ...
        result.errors.push(`Failed to seed pattern ${pattern.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  ...
  private async seedComplianceRules(
    ...
    const complianceRules = [
      {
        ...
        title: locale === 'nb-NO' ? 'NSM Sikkerhetskklassifisering' : 'NSM Security Classification',
        description: locale === 'nb-NO' 
      ...
      },
      {
        ...
        title: locale === 'nb-NO' ? 'WCAG Tilgjengelighet' : 'WCAG Accessibility',
        description: locale === 'nb-NO'
    ...
    for (const rule of complianceRules) {
      try {
        ...
        const collection = await this.vectorStore.getCollection('xala_compliance_rules');
        if (override) {
          await collection.update([vectorDoc]);
        } else {
          await collection.add([vectorDoc]);
        ...
        result.errors.push(`Failed to seed compliance rule ${rule.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
    ...
    for (const pattern of this.BUILT_IN_PATTERNS) {
      for (let i = 0; i < pattern.examples.length; i++) {
        try {
          ...
          result.errors.push(`Failed to seed example from pattern ${pattern.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }
    }
  ...
  private validatePattern(pattern: ICodePattern): boolean {
    ...
    if (pattern.compliance) {
      const { nsmClassification, gdprCompliant, wcagLevel } = pattern.compliance;
      
      if (!['OPEN', 'RESTRICTED', 'CONFIDENTIAL', 'SECRET'].includes(nsmClassification)) {
        return false;
      }
  ...
  private isPatternFile(content: string, filename: string): boolean {
    ...
    if (exampleIndicators.some(indicator => lowerFilename.includes(indicator))) {
      return false;
    }
