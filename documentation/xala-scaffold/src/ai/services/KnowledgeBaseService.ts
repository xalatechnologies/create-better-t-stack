...
import { LocaleCode, NorwegianCompliance, NSMClassification, GDPRDataCategory } from '../../types/compliance.js';
  ...
  private readonly BUILT_IN_COMPLIANCE_RULES: Partial<IComplianceRule>[] = [
    {
      ...
      description: 'All components must include NSM security classification data attributes',
      category: 'nsm',
      severity: 'error',
      pattern: /data-nsm-classification=["']?(OPEN|RESTRICTED|CONFIDENTIAL|SECRET)["']?/,
      explanation: 'Norwegian NSM requires security classification on all UI components',
      examples: {
        good: ['<div data-nsm-classification="OPEN">', '<button data-nsm-classification="RESTRICTED">'],
        bad: ['<div>', '<button>']
      },
    ...
    },
    {
      ...
      explanation: 'GDPR requires explicit consent for personal data processing',
      examples: {
        good: ['const { hasConsent } = useGDPRConsent()', 'if (gdprConsent.dataProcessing) {'],
    ...
    },
    {
      ...
      examples: {
        good: ['<button aria-label="Save changes">', '<input aria-describedby="help-text">'],
        bad: ['<button>', '<input>']
      },
      applicableLanguages: ['typescript', 'javascript', 'jsx', 'tsx'],
      isActive: true
    ...
    },
    {
      ...
      examples: {
        good: ['auditLogger.log({ action: "user_update" })', 'await audit.log(operation)'],
  ...
  private readonly BUILT_IN_PATTERNS: Partial<ICodePattern>[] = [
    {
      ...
      description: 'Button component with NSM classification, WCAG AAA accessibility, and Norwegian language support',
      category: 'component',
...
export const Button: React.FC<ButtonProps> = ({
  ...
  return (
    ...
    >
      ...
      usage: 'Norwegian compliant button with NSM classification and WCAG AAA accessibility',
      qualityScore: 95,
      usageCount: 0
    },
    {
      ...
      description: 'Form pattern with GDPR consent management and Norwegian validation',
      category: 'form',
...
export const GDPRForm: React.FC = () => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-nsm-classification="RESTRICTED">
      ...
      usage: 'GDPR compliant form with Norwegian validation and consent management',
      qualityScore: 90,
      usageCount: 0
    }
  ];
  ...
  constructor(
    ...
    this.knowledgeBasePath = this.config.get<string>('ai.knowledgeBase.path', './knowledge-base');
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.error('Failed to initialize Knowledge Base Service', error as Error);
      throw error;
    }
  }
  ...
  async dispose(): Promise<void> {
    try {
      ...
      this.logger.error('Error disposing Knowledge Base Service', error as Error);
      throw error;
    }
  }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      const hasContent = this.storage.patterns.size > 0 || 
  ...
  async addPattern(pattern: ICodePattern): Promise<void> {
    try {
      this.logger.debug('Adding code pattern', { id: pattern.id, name: pattern.name });
      ...
      this.logger.error('Failed to add pattern', error as Error, { patternId: pattern.id });
      throw error;
    }
  }
  ...
  async getPattern(id: string): Promise<ICodePattern | null> {
    try {
      ...
      this.logger.error('Failed to get pattern', error as Error, { patternId: id });
      throw error;
    }
  }
   ...
   */
  async searchPatterns(criteria: IPatternSearchCriteria): Promise<ICodePattern[]> {
    try {
      ...
      if (criteria.category) {
        patterns = patterns.filter(pattern => pattern.category === criteria.category);
      }
      ...
      if (criteria.language) {
        patterns = patterns.filter(pattern => pattern.language === criteria.language);
      }

      if (criteria.framework) {
        patterns = patterns.filter(pattern => pattern.framework === criteria.framework);
      }
      ...
      if (criteria.minQualityScore) {
        patterns = patterns.filter(pattern => pattern.qualityScore >= criteria.minQualityScore!);
      }
      ...
      patterns.sort((a, b) => {
        switch (sortBy) {
          ...
          case 'relevance':
          default:
            return b.qualityScore * (b.usageCount + 1) - a.qualityScore * (a.usageCount + 1);
        }
      });
  ...
  async removePattern(id: string): Promise<void> {
    try {
      ...
      this.logger.error('Failed to remove pattern', error as Error, { patternId: id });
      throw error;
    }
  }
  ...
  async addComplianceRule(rule: IComplianceRule): Promise<void> {
    try {
      this.logger.debug('Adding compliance rule', { id: rule.id, name: rule.name });
      ...
      this.logger.error('Failed to add compliance rule', error as Error, { ruleId: rule.id });
      throw error;
    }
  }
   ...
   */
  async getComplianceRules(classification?: string): Promise<IComplianceRule[]> {
    try {
    ...
    try {
      ...
      for (const rule of rules) {
        ...
        if (rule.examples.good.length > 0) {
          suggestions.push(`${rule.name}: Consider using patterns like: ${rule.examples.good[0]}`);
        }
      }
  ...
  async addExample(example: ICodeExample): Promise<void> {
    try {
      this.logger.debug('Adding code example', { id: example.id, title: example.title });
      ...
      this.logger.error('Failed to add example', error as Error, { exampleId: example.id });
      throw error;
    }
  }
   ...
   */
  async getExamples(category: string, locale?: LocaleCode): Promise<ICodeExample[]> {
    try {
  ...
  async seedFromDirectory(directoryPath: string): Promise<void> {
    try {
      this.logger.info('Seeding knowledge base from directory', { path: directoryPath });
      ...
      for (const file of files) {
        ...
        if (stats.isFile && (file.endsWith('.ts') || file.endsWith('.tsx') || file.endsWith('.js') || file.endsWith('.jsx'))) {
          try {
            ...
            const extracted = await this.extractKnowledgeFromFile(content, file, filePath);
  ...
  async importKnowledgeBase(data: string): Promise<void> {
    try {
      ...
      if (!importData.version || !importData.patterns || !importData.complianceRules || !importData.examples) {
    ...
    try {
      ...
      for (const file of patternFiles) {
        if (file.endsWith('.json')) {
          try {
            const content = await this.fileSystem.read(path.join(patternsDir, file));
      ...
      for (const file of exampleFiles) {
        if (file.endsWith('.json')) {
          try {
            const content = await this.fileSystem.read(path.join(examplesDir, file));
  ...
  private async savePattern(pattern: ICodePattern): Promise<void> {
    const filePath = path.join(this.knowledgeBasePath, 'patterns', `${pattern.id}.json`);
  ...
  private async saveComplianceRule(rule: IComplianceRule): Promise<void> {
    const filePath = path.join(this.knowledgeBasePath, 'rules', `${rule.id}.json`);
    const content = JSON.stringify(rule, null, 2);
    await this.fileSystem.write(filePath, content);
  }

  private async saveExample(example: ICodeExample): Promise<void> {
    const filePath = path.join(this.knowledgeBasePath, 'examples', `${example.id}.json`);
    const content = JSON.stringify(example, null, 2);
    await this.fileSystem.write(filePath, content);
  }

  private async deletePersistedPattern(id: string): Promise<void> {
    const filePath = path.join(this.knowledgeBasePath, 'patterns', `${id}.json`);
    try {
  ...
  private validatePattern(pattern: ICodePattern): void {
    ...
    if (!pattern.language || !['typescript', 'javascript', 'jsx', 'tsx'].includes(pattern.language)) {
  ...
  private validateExample(example: ICodeExample): void {
    ...
    if (!example.language || !['typescript', 'javascript', 'jsx', 'tsx'].includes(example.language)) {
      throw new Error('Example must have a supported language');
    }
  }

  private isRuleApplicableToClassification(rule: IComplianceRule, classification: string): boolean {
  ...
  private async extractKnowledgeFromFile(
    ...
    filename: string, 
    filepath: string
  ): Promise<{ type: 'pattern' | 'example' | 'none'; pattern?: ICodePattern; example?: ICodeExample }> {
    // Simple heuristic to determine if file contains a pattern or example
    
    // Look for component patterns
    if (content.includes('interface') && content.includes('Props') && content.includes('export')) {
    ...
    if (filename.includes('example') || filename.includes('demo') || content.includes('// Example:')) {
  ...
  private detectCompliance(content: string): NorwegianCompliance {
    ...
    let nsmClassification: 'OPEN' | 'RESTRICTED' | 'CONFIDENTIAL' | 'SECRET' = 'OPEN';
  ...
  private extractCategory(content: string, filename: string): string {
    ...
    if (filename.includes('layout') || content.includes('layout')) return 'layout';
  ...
  private extractFeatures(content: string): string[] {
    ...
    if (content.includes('data-nsm-classification')) features.push('NSM compliance');
