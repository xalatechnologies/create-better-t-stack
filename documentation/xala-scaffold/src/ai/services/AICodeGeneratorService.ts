...
import { LocaleCode, NorwegianCompliance, NSMClassification } from '../../types/compliance.js';

/**
 * AI Code Generator Service Implementation
 */
export class AICodeGeneratorService extends EventEmitter implements IAICodeGeneratorService {
...
Provide specific improvement suggestions with code examples.`
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.error('Failed to initialize AI Code Generator Service', error as Error);
      throw error;
    }
  }
  ...
  async dispose(): Promise<void> {
    try {
      ...
      this.logger.error('Error disposing AI Code Generator Service', error as Error);
      throw error;
    }
  }
   ...
   */
  async generateComponentWithAI(requirements: INaturalLanguageRequirements): Promise<ICodeGenerationResult> {
   ...
   */
  async generatePageWithAI(requirements: INaturalLanguageRequirements): Promise<ICodeGenerationResult> {
   ...
   */
  async generateAPIWithAI(requirements: INaturalLanguageRequirements): Promise<ICodeGenerationResult> {
   ...
   */
  async analyzeCode(code: string, filePath: string): Promise<ICodeAnalysisResult> {
    ...
    try {
      ...
      const complianceRules = await this.knowledgeBaseService.getComplianceRules(
        projectContext?.compliance?.nsmClassification
      );
      const complianceResult = await this.knowledgeBaseService.validateCompliance(code, complianceRules);
   ...
   */
  async suggestImprovements(code: string, context?: any): Promise<IImprovementSuggestion[]> {
    try {
      ...
      const prompt = this.buildImprovementPrompt(code, analysis, ragContext, locale);
      ...
      const suggestions = this.parseImprovementSuggestions(llmResponse.text, analysis);
      ...
      this.logger.error('Failed to generate improvement suggestions', error as Error);
      throw error;
    }
  }
   ...
   */
  async chatAssistant(message: string, context?: IConversationContext): Promise<IAssistantResponse> {
    try {
      ...
      this.logger.error('Failed to generate assistant response', error as Error);
      throw error;
    }
  }
  ...
  async validateAndRefineCode(
    ...
    try {
      ...
      const needsRefinement = this.assessRefinementNeeds(analysis, requirements);
      ...
      const prompt = this.buildRefinementPrompt(code, analysis, requirements, locale);

      // Generate refined code
      const llmResponse = await this.llmService.generateCode(prompt, undefined, {
  ...
  private async generateCodeWithAI(requirements: INaturalLanguageRequirements): Promise<ICodeGenerationResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      const projectContext = this.contextManagerService.getProjectContext();
      const userPreferences = this.contextManagerService.getUserPreferences();
      const locale = requirements.locale || projectContext?.locale || userPreferences.defaultLocale;
      const compliance = requirements.compliance || projectContext?.compliance || {
      ...
      const llmResponse = await this.llmService.generateCode(prompt, {
        ...
        compliance,
        locale,
        includeTests: requirements.includeTests || userPreferences.autoTesting,
        includeDocumentation: requirements.includeDocumentation || true,
        temperature: 0.3
      });

      // Parse generated files
      const files = await this.parseGeneratedFiles(llmResponse.code, requirements);

      // Validate compliance
      const complianceRules = await this.knowledgeBaseService.getComplianceRules(
        compliance.nsmClassification
      );
      const complianceResult = await this.knowledgeBaseService.validateCompliance(
        llmResponse.code,
        complianceRules
      );
      ...
      if (!complianceResult.isValid || llmResponse.qualityMetrics.overallScore < 80) {
        try {
          const refinementResult = await this.validateAndRefineCode(llmResponse.code, requirements);
          if (refinementResult.qualityImprovementScore > 10) {
            finalFiles = await this.parseGeneratedFiles(refinementResult.refinedCode, requirements);
          }
        } catch (refinementError) {
          this.logger.warn('Code refinement failed, using original code', { 
            error: refinementError instanceof Error ? refinementError.message : 'Unknown error'
          });
        }
      }
  ...
  private buildGenerationPrompt(
    ...
    const prompts = this.NORWEGIAN_PROMPTS[locale] || this.NORWEGIAN_PROMPTS['en-US'];
    
    let prompt = prompts.systemPrompt + '\n\n';
    prompt += prompts.codeGenerationPrompt.replace('{classification}', compliance.nsmClassification) + '\n\n';
  ...
  private buildConversationMessages(
    ...
    if (ragContext.documents.length > 0) {
      const contextInfo = ragContext.documents
        .slice(0, 2)
        .map((doc: any) => `${doc.document.metadata.category}: ${doc.document.content.substring(0, 200)}`)
        .join('\n');
  ...
  private async parseGeneratedFiles(
    ...
    const mainFile: IGeneratedFile = {
      ...
      language: requirements.outputFormat === 'javascript' ? 'javascript' : 'typescript',
    ...
    if (requirements.includeTests && generatedCode.includes('test(') || generatedCode.includes('it(')) {
  ...
  private extractTestCode(generatedCode: string): string | null {
    ...
    const testBlockMatch = generatedCode.match(/\/\*\* TEST FILE \*\*\/([\s\S]*?)\/\*\* END TEST \*\*\//);
    if (testBlockMatch) {
  ...
  private parseImprovementSuggestions(response: string, analysis: ICodeAnalysisResult): IImprovementSuggestion[] {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      if (section.startsWith('**') || section.includes('forbedringsforslag') || section.includes('improvement')) {
        suggestions.push({
          id: `suggestion-${i}`,
          title: section.split('\n')[0].replace(/\*+/g, '').trim(),
          description: section,
  ...
  private parseRefinementResult(
    ...
    return {
      ...
      complianceImprovements: ['NSM classification added', 'ARIA labels enhanced'],
      qualityImprovementScore: 15,
      explanation: 'Code has been refined to meet Norwegian compliance requirements and improve quality.'
    };
  }
  ...
  private calculateCodeQualityMetrics(code: string): ICodeQualityMetrics {
    ...
    return {
      ...
      codeSmells: [],
      overallScore: Math.max(20, 100 - complexity - Math.floor(lines.length / 50))
    };
  }
  ...
  private calculateComplexity(code: string): number {
    const complexityIndicators = [
      ...
      /switch\s*\(/g, /case\s+/g, /catch\s*\(/g, /&&/g, /\|\|/g, /\?.*:/g
    ];
  ...
  private assessRefinementNeeds(analysis: ICodeAnalysisResult, requirements: INaturalLanguageRequirements): boolean {
    return analysis.compliance.score < 80 || 
