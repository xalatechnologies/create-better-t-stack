...
import {
	ICodePattern as IPatternMatcherPattern,
	PatternMatcher,
} from "../../rag/pattern-matching/PatternMatcher.js";
import {
	CodeRetriever,
	ICodeQuery,
} from "../../rag/retrieval/CodeRetriever.js";
import {
	ChromaDBStore,
	IVectorDocument,
} from "../../rag/vector-store/ChromaDBStore.js";
...
export class RAGService extends EventEmitter implements IRAGService {
	...
	constructor(
		...
		this.vectorStore =
			vectorStore || new ChromaDBStore(this.logger, this.config);
		this.embeddingGenerator =
			embeddingGenerator || new EmbeddingGenerator(this.logger, this.config);
		this.codeRetriever =
			codeRetriever ||
			new CodeRetriever(
				this.vectorStore,
	...
	async addToVectorStore(
		...
		try {
			...
			for (const doc of documents) {
				...
				if (generateEmbeddings) {
					const embeddingResult =
						await this.embeddingGenerator.generateEmbedding(doc.content, {
			...
			const docsByCollection =
				this.groupVectorDocumentsByCollection(vectorDocuments);
	...
	async retrieveContext(
		...
		try {
			...
			const enhancedQuery = this.enhanceQueryWithComplianceKeywords(
				query,
	...
	async retrieveCodePatterns(
		...
		try {
			...
			this.logger.debug("Code patterns retrieved", {
				...
				avgScore:
					patterns.length > 0
						? patterns.reduce((sum, p) => sum + p.qualityScore, 0) /
							patterns.length
						: 0,
			});
	...
	async updateKnowledgeBase(patterns: ICodePattern[]): Promise<void> {
		try {
			...
			const documents = patterns.map((pattern) =>
				this.convertPatternToDocument(pattern),
			);
	...
	async getIndexStats(): Promise<IIndexStats> {
		try {
			...
			const stats: IIndexStats = {
				...
				embeddingDimensions:
					this.embeddingGenerator.getCacheStats().maxSize || 384,
	...
	private groupVectorDocumentsByCollection(
		documents: IVectorDocument[],
	): Map<string, IVectorDocument[]> {
	...
	private createChromaClient(): IChromaClient {
		const chromaUrl = this.config.get<string>(
			"ai.chroma.url",
			"http://localhost:8000",
		);
		...
		return {
			...
			async deleteCollection(name: string) {
				const response = await fetch(
					`${chromaUrl}/api/v1/collections/${name}`,
					{
						method: "DELETE",
					},
				);
	...
	private convertChromaResults(chromaResults: any): ISearchResult[] {
		...
		for (let i = 0; i < chromaResults.documents[0].length; i++) {
			const document: IDocument = {
				...
				embedding: chromaResults.embeddings
					? chromaResults.embeddings[0][i]
					: undefined,
			};
	...
	private enhanceQueryWithComplianceKeywords(
		...
		const keywords =
			this.NORWEGIAN_COMPLIANCE_KEYWORDS[locale] ||
			this.NORWEGIAN_COMPLIANCE_KEYWORDS["en-US"];
		...
		if (
			...
			enhancedQuery +=
				...
				keywords
					.filter(
						(k) => k.includes("tilgjengelighet") || k.includes("accessibility"),
					)
					.join(" ");
		}
		...
		if (
			...
			enhancedQuery +=
				" " +
				keywords
					.filter((k) => k.includes("gdpr") || k.includes("personvern"))
					.join(" ");
		}
	...
	private buildContextualFilters(
		projectContext?: IProjectContext,
	): ISearchFilters {
	...
	private generateContextSummary(
		...
		const frameworks = [
			...new Set(
				topResults.map((r) => r.document.metadata.framework).filter(Boolean),
			),
		];
	...
	private async generateContextSuggestions(
		...
		const types = [
			...new Set(searchResults.map((r) => r.document.metadata.type)),
		];
		const frameworks = [
			...new Set(
				searchResults.map((r) => r.document.metadata.framework).filter(Boolean),
			),
		];

		if (types.includes("pattern")) {
			suggestions.push(
				"Consider using established code patterns for consistency",
			);
		}
		...
		if (frameworks.length > 0) {
			suggestions.push(
				`Consider framework-specific best practices for ${frameworks.join(" and ")}`,
			);
		}
