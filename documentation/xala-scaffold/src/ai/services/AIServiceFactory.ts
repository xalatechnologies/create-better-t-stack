...
export class AIServiceFactory implements IAIServiceFactory {
  ...
  updateConfiguration(updates: Partial<IServiceConfiguration>): void {
    ...
    this.logger.info('Service configuration updated', { updates: Object.keys(updates) });
  }
  ...
  private loadConfiguration(): IServiceConfiguration {
    const config: IServiceConfiguration = {
      llm: {
        provider: this.config.get<'ollama' | 'localai'>('ai.llm.provider', 'ollama'),
        baseUrl: this.config.get<string>('ai.llm.baseUrl', 'http://localhost:11434'),
        defaultModel: this.config.get<string>('ai.llm.defaultModel', 'codellama:7b'),
        timeout: this.config.get<number>('ai.llm.timeout', 30000)
      },
      rag: {
        provider: this.config.get<'chromadb' | 'local'>('ai.rag.provider', 'chromadb'),
        url: this.config.get<string>('ai.rag.url', 'http://localhost:8000'),
        embeddingModel: this.config.get<string>('ai.rag.embeddingModel', 'all-MiniLM-L6-v2'),
        dimensions: this.config.get<number>('ai.rag.dimensions', 384)
      },
      knowledgeBase: {
        path: this.config.get<string>('ai.knowledgeBase.path', './knowledge-base'),
        autoSeed: this.config.get<boolean>('ai.knowledgeBase.autoSeed', true),
        exportPath: this.config.get<string>('ai.knowledgeBase.exportPath', './knowledge-base-export.json')
      },
      context: {
        storePath: this.config.get<string>('ai.context.storePath', './context-store'),
        maxHistoryEntries: this.config.get<number>('ai.context.maxHistoryEntries', 1000),
        enableLearning: this.config.get<boolean>('ai.context.enableLearning', true)
      },
      compliance: {
        strictMode: this.config.get<boolean>('ai.compliance.strictMode', true),
        defaultClassification: this.config.get<string>('ai.compliance.defaultClassification', 'OPEN'),
        enableAuditLogging: this.config.get<boolean>('ai.compliance.enableAuditLogging', true)
      }
    };
  ...
  private async seedKnowledgeBase(knowledgeBaseService: IKnowledgeBaseService): Promise<void> {
    try {
  ...
  private validateConfiguration(): void {
    ...
    if (!config.llm.baseUrl || !config.llm.provider) {
      throw new Error('Invalid LLM configuration: baseUrl and provider are required');
    }

    // Validate RAG configuration
    if (!config.rag.url || !config.rag.embeddingModel) {
      throw new Error('Invalid RAG configuration: url and embeddingModel are required');
    }

    // Validate paths
    if (!config.knowledgeBase.path || !config.context.storePath) {
      throw new Error('Invalid path configuration: knowledgeBase.path and context.storePath are required');
    }
...
export function getAIServiceFactory(): AIServiceFactory {
  if (!factoryInstance) {
    throw new Error('AI Service Factory not initialized. Call createAIServiceFactory first.');
  }
  return factoryInstance;
}