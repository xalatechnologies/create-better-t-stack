...
import { ILoggingService, IConfigurationService, IFileSystemService } from '../../architecture/interfaces.js';
 ...
 * Context Manager Service Implementation
 */
export class ContextManagerService extends EventEmitter implements IContextManagerService {
  ...
  constructor(
    ...
    this.contextStorePath = this.config.get<string>('ai.context.storePath', './context-store');
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.error('Failed to initialize Context Manager Service', error as Error);
      throw error;
    }
  }
  ...
  async dispose(): Promise<void> {
    try {
      ...
      this.logger.error('Error disposing Context Manager Service', error as Error);
      throw error;
    }
  }
  ...
  async analyzeProject(projectPath: string): Promise<IProjectContext> {
    try {
      ...
      const compliance = await this.detectNorwegianCompliance(projectPath, analysis);
      ...
      this.emit('projectAnalyzed', { projectPath, projectType: analysis.projectType });
      ...
      this.logger.error('Failed to analyze project', error as Error, { projectPath });
      throw error;
    }
  }
  ...
  async updateProjectContext(context: Partial<IProjectContext>): Promise<void> {
    try {
      ...
      this.logger.debug('Updating project context', { updates: Object.keys(context) });
      ...
      this.emit('projectContextUpdated', { context: this.currentProjectContext });
   ...
   */
  async updateUserPreferences(preferences: Partial<IUserPreferences>): Promise<void> {
    try {
      this.logger.debug('Updating user preferences', { updates: Object.keys(preferences) });
      ...
      this.emit('userPreferencesUpdated', { preferences: this.userPreferences });
  ...
  async addGenerationHistory(entry: IGenerationHistoryEntry): Promise<void> {
    try {
      ...
      if (this.generationHistory.length > this.maxHistoryEntries) {
        this.generationHistory = this.generationHistory.slice(0, this.maxHistoryEntries);
      }
      ...
      this.logger.error('Failed to add generation history entry', error as Error);
      throw error;
    }
  }
   ...
   */
  async getGenerationHistory(limit: number = 50): Promise<IGenerationHistoryEntry[]> {
    try {
      const history = this.generationHistory.slice(0, limit);
      
      this.logger.debug('Retrieved generation history', { count: history.length });
   ...
   */
  async recordUserFeedback(generationId: string, feedback: IUserFeedback): Promise<void> {
    try {
      this.logger.debug('Recording user feedback', { generationId, rating: feedback.rating });
  ...
  private async loadUserPreferences(): Promise<void> {
    try {
      const preferencesPath = path.join(this.contextStorePath, 'user-preferences.json');
  ...
  private async loadGenerationHistory(): Promise<void> {
    try {
      const historyPath = path.join(this.contextStorePath, 'generation-history.json');
  ...
  private async saveProjectContext(context: IProjectContext): Promise<void> {
    try {
      const contextPath = path.join(this.contextStorePath, 'project-context.json');
  ...
  private async performProjectAnalysis(projectPath: string): Promise<IProjectAnalysisResult> {
  ...
  private async analyzeProjectStructure(projectPath: string): Promise<IProjectStructure> {
      ...
      if (structure.srcDirectory) {
        ...
        if (srcExists) {
          ...
          for (const file of srcFiles) {
            ...
            if (stats.isDirectory) {
              if (file === 'components') {
                structure.componentsDirectory = path.join(structure.srcDirectory, file);
              } else if (file === 'pages') {
                structure.pagesDirectory = path.join(structure.srcDirectory, file);
              } else if (file === 'api') {
                structure.apiDirectory = path.join(structure.srcDirectory, file);
              }
            }
          }
        }
  ...
  private async detectCodeStyle(projectPath: string): Promise<ICodeStylePreferences> {
  ...
  private async analyzeCodeFiles(projectPath: string, metrics: IProjectMetrics): Promise<void> {
        ...
        if (this.isCodeFile(file)) {
          ...
          for (const line of lines) {
            ...
            if (file.includes('test') || file.includes('spec') || 
                trimmed.includes('test(') || trimmed.includes('it(') || trimmed.includes('describe(')) {
              metrics.testLines++;
            }
          }
        }
  ...
  private calculateMaintainabilityIndex(metrics: IProjectMetrics): number {
    ...
    const avgLinesPerFile = metrics.totalLines / Math.max(1, metrics.codeLines / 100);
  ...
  private estimateTechnicalDebt(metrics: IProjectMetrics, structure: IProjectStructure): number {
  ...
  private async detectNorwegianCompliance(
    ...
    try {
      ...
      if (deps['@xala-technologies/norwegian-compliance'] || 
  ...
  private async detectLocalePreferences(
    ...
    try {
      ...
      const i18nFiles = ['i18n.js', 'i18n.ts', 'next-i18next.config.js', 'locales.json'];
      ...
      for (const file of i18nFiles) {
        ...
        if (exists) {
          ...
          if (content.includes('nb-NO') || content.includes('no-NO') || content.includes('nor')) {
      ...
      if (exists) {
        ...
        if (content.includes('norway') || content.includes('norwegian') || content.includes('norge')) {
          return 'nb-NO' as LocaleCode;
        }
      }
    } catch (error) {
  ...
  private async checkForNSMIndicators(
    srcPath: string
  ): Promise<{ hasRestricted: boolean; hasConfidential: boolean; hasSecret: boolean }> {
    // Simplified check - would recursively check all files in real implementation
    const result = { hasRestricted: false, hasConfidential: false, hasSecret: false };
    ...
    try {
      ...
      for (const file of files.slice(0, 10)) {
        if (this.isCodeFile(file)) {
          ...
          if (content.includes('gdpr') || content.includes('GDPR') || 
              content.includes('consent') || content.includes('privacy')) {
            return true;
          }
        }
      }
  ...
  private async checkForWCAGIndicators(projectPath: string): Promise<'A' | 'AA' | 'AAA' | undefined> {
    try {
      const files = await this.fileSystem.list(projectPath);
      ...
      for (const file of files.slice(0, 10)) {
        if (this.isCodeFile(file)) {
          ...
          if (content.includes('wcag') || content.includes('WCAG')) hasWCAG = true;
          if (content.includes('aria-label') || content.includes('aria-describedby')) hasAriaLabels = true;
          if (content.includes('screen reader') || content.includes('sr-only')) hasScreenReaderSupport = true;
        }
      }
  ...
  private async analyzeGenerationHistory(): Promise<IAdaptationInsights> {
    ...
    try {
      ...
      const modelStats = new Map<string, { total: number; successful: number; avgRating: number; ratings: number[] }>();
      ...
      for (const entry of recentHistory) {
        ...
        if (entry.success) {
          ...
          successfulPrompts.set(prompt, (successfulPrompts.get(prompt) || 0) + 1);
        }

        // Track model performance
        if (!modelStats.has(entry.model)) {
          modelStats.set(entry.model, { total: 0, successful: 0, avgRating: 0, ratings: [] });
        }
      ...
      for (const [model, stats] of modelStats.entries()) {
        ...
        const performance: IModelPerformanceInsight = {
          ...
          recommendations: this.generateModelRecommendations(model, successRate, avgRating)
        };
  ...
  private identifyCommonIssues(history: IGenerationHistoryEntry[], model: string): string[] {
    const issues: string[] = [];
    
    const modelEntries = history.filter(entry => entry.model === model && !entry.success);
  ...
  private generateModelRecommendations(model: string, successRate: number, avgRating: number): string[] {
    const recommendations: string[] = [];
    
    if (successRate < 0.7) {
      recommendations.push('Consider using more specific prompts for better results');
    }
    
    if (avgRating < 3) {
      recommendations.push('Review generated code quality and consider alternative models');
    }
    
    if (model.includes('codellama')) {
      recommendations.push('CodeLlama works best with detailed technical requirements');
    }
  ...
  private identifyProblematicAreas(history: IGenerationHistoryEntry[]): string[] {
    const areas: string[] = [];
  ...
  private identifyImprovementAreas(history: IGenerationHistoryEntry[]): string[] {
    const areas: string[] = [];
    
    const lowComplianceCount = history.filter(entry => entry.complianceScore < 80).length;
    const lowQualityCount = history.filter(entry => entry.qualityScore < 70).length;
    const lowRatingCount = history.filter(entry => entry.userFeedback && entry.userFeedback.rating < 3).length;
  ...
  private isConfigFile(filename: string): boolean {
    ...
    const configNames = [
      'package.json', 'tsconfig.json', '.eslintrc', '.prettierrc', 'jest.config',
      'vite.config', 'webpack.config', 'next.config', 'tailwind.config'
    ];
    
    return configNames.some(name => filename.startsWith(name)) ||
           (filename.startsWith('.') && configExtensions.some(ext => filename.endsWith(ext)));
  }
