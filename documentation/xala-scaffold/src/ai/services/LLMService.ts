...
7. Follow SOLID principles`
  ...
  constructor(
    ...
    this.baseUrl = this.config.get<string>('ai.ollama.baseUrl', 'http://localhost:11434');
  ...
  async listAvailableModels(): Promise<ILLMModel[]> {
    try {
      ...
      return response.models.map((model: any) => this.mapOllamaModelToInterface(model));
    } catch (error) {
   ...
   */
  async loadModel(modelName: string, options: IModelLoadOptions = {}): Promise<void> {
    try {
      ...
      this.logger.error('Failed to load model', error as Error, { model: modelName });
  ...
  async unloadModel(modelName: string): Promise<void> {
    try {
      ...
      this.logger.error('Failed to unload model', error as Error, { model: modelName });
      throw error;
    }
  }
   ...
   */
  async generateText(prompt: string, options: IGenerationOptions = {}): Promise<ILLMResponse> {
    ...
    try {
      ...
      // Update stats
      this.updateModelStats(this.currentModel.name, Date.now() - startTime, true);
      ...
      this.updateModelStats(this.currentModel?.name || 'unknown', Date.now() - startTime, false);
  ...
  async generateCode(
    ...
    try {
      ...
      const locale = options.locale || context?.locale || 'nb-NO' as LocaleCode;
      const compliance = options.compliance || context?.compliance || { 
      ...
      const systemPrompt = this.compliancePrompts[locale]?.system || this.compliancePrompts['en-US'].system;
      const codePrompt = this.compliancePrompts[locale]?.codeGeneration || this.compliancePrompts['en-US'].codeGeneration;
      ...
      const code = this.extractCodeFromResponse(response.text, options.language);
  ...
  async chat(messages: IChatMessage[], options: IChatOptions = {}): Promise<IChatResponse> {
    ...
    try {
      ...
      const usage: ITokenUsage = {
        promptTokens: this.estimateTokens(chatMessages.map(m => m.content).join(' ')),
  ...
  async getModelStats(): Promise<IModelStats> {
    ...
    if (!stats) {
      throw new Error(`No statistics available for model ${this.currentModel.name}`);
    }
  ...
  private async testConnection(): Promise<void> {
    try {
      ...
      throw new Error(`Cannot connect to Ollama at ${this.baseUrl}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async refreshModelList(): Promise<void> {
    try {
      ...
      this.logger.warn('Failed to refresh model list', { error: error instanceof Error ? error.message : 'Unknown error' });
    }
  }

  private mapOllamaModelToInterface(ollamaModel: any, isLoaded = false): ILLMModel {
    ...
    return {
      ...
      description: ollamaModel.details?.description || `${ollamaModel.name} model`,
  ...
  private isCodeModel(modelName: string): boolean {
    const codeModels = ['codellama', 'deepseek-coder', 'starcoder', 'phind-codellama', 'wizard-coder'];
  ...
  private hasNorwegianSupport(modelName: string): boolean {
    ...
    return norwegianModels.some(model => modelName.toLowerCase().includes(model));
  }
  ...
  private calculateCodeQuality(code: string): ICodeQualityMetrics {
    ...
    return {
      ...
      codeSmells: [],
      overallScore: Math.max(0, 100 - complexity - Math.floor(lines.length / 50))
    };
  }
  ...
  private calculateComplianceScore(code: string, compliance: NorwegianCompliance): number {
    let score = 0;
  ...
  private updateModelStats(modelName: string, duration: number, success: boolean): void {
    ...
    if (success) {
      stats.averageResponseTime = (stats.averageResponseTime * (stats.totalGenerations - 1) + duration) / stats.totalGenerations;
    } else {
      stats.errorCount++;
    }
