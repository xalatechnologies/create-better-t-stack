...
export class InteractiveAIAssistant extends EventEmitter {
	...
	async startSession(
		...
		try {
			...
			if (options.projectPath) {
				this.context.currentProject = await this.detectProjectInfo(
					options.projectPath,
				);
			}
			if (options.locale) {
	...
	private displayWelcome(): void {
		...
		if (this.context.locale === "nb-NO") {
			console.log(
				chalk.green(
					"Velkommen! Jeg er din AI-assistent for norsk-kompatibel utvikling.",
				),
			);
			...
			console.log(
				chalk.gray(
					'Skriv "hjelp" for å se alle kommandoer, eller "exit" for å avslutte.',
				),
			);
		} else {
			console.log(
				chalk.green(
					"Welcome! I am your AI assistant for Norwegian-compliant development.",
				),
			);
			console.log("I can help you with:");
			console.log(
				"  • Generate code with Norwegian compliance (NSM, GDPR, WCAG)",
			);
			...
			console.log();
			console.log(
				chalk.gray('Type "help" to see all commands, or "exit" to quit.'),
			);
		}
	...
	private getPrompt(): string {
		const icon =
			this.context.compliance.nsmClassification !== "OPEN" ? "🔒" : "💬";
	...
	private async conversationLoop(): Promise<void> {
		return new Promise((resolve) => {
			...
			this.rl.on("line", async (input) => {
				...
				if (
					["exit", "quit", "bye", "avslutt"].includes(
	...
	private async recognizeIntent(message: string): Promise<IIntentRecognition> {
		...
		try {
			...
			if (
				analysis.summary.includes("generate") ||
				analysis.summary.includes("create")
			) {
				return { intent: "CODE_GENERATION", confidence: 0.6, entities: {} };
			}
			if (
				analysis.summary.includes("analyze") ||
				analysis.summary.includes("check")
			) {
				return { intent: "CODE_ANALYSIS", confidence: 0.6, entities: {} };
			}
		} catch (error) {
			this.logger.warn(
				"AI intent recognition failed, using fallback",
				error as Error,
			);
		}
	...
	private extractEntities(
		...
		const complianceMatch = message.match(
			/\b(OPEN|RESTRICTED|CONFIDENTIAL|SECRET)\b/i,
		);
		if (complianceMatch) {
		...
		const langMatch = message.match(
			/\b(typescript|javascript|react|vue|angular)\b/i,
		);
		if (langMatch) {
		...
		try {
			...
			const result = await aiService.generateCode({
				...
				compliance: entities.compliance
					? {
							...this.context.compliance,
							nsmClassification: entities.compliance,
						}
					: this.context.compliance,
	...
	private async handleCodeAnalysis(
		...
		try {
			...
			const complianceIssues =
				await this.patternMatcher!.detectComplianceIssues(
					code,
					this.context.locale,
				);

			let response =
				this.context.locale === "nb-NO"
	 ...
	 */
	private async handleCodeRefactoring(
		message: string,
		entities: Record<string, any>,
	): Promise<string> {
		try {
			...
			const code = await this.fileSystem.readFile(entities.filePath);
			const improvements = await aiService.improveCode(
				code,
				this.context.compliance,
			);
	...
	private async handleTesting(
		...
		try {
			...
			const template = this.promptEngine!.getTemplate(
				templateId,
				this.context.locale,
			);
	...
	private async learnFromInteraction(
		...
		try {
			const contextManager =
				this.aiServiceFactory!.createContextManagerService();
