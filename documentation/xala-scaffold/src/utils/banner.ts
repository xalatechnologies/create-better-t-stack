...
import { getDescription, getVersion } from "./package.js";
...
export async function displayBanner(
	...
	try {
		...
		if (showVersion) {
			...
			console.log(chalk.gray("Version:"), chalk.bold.white(version));
		}
...
export function displayCompletionBanner(
	projectName: string,
	projectPath: string,
): void {
	console.log();
	...
	console.log(
		chalk.white(
			`  cd ${path.relative(process.cwd(), projectPath) || projectName}`,
		),
	);
...
export async function displayWelcome(): Promise<void> {
	...
	console.log(
		chalk.gray(
			"This tool helps you scaffold enterprise-grade frontend projects.",
		),
	);
	console.log();
