...
export function getExitCode(error: unknown): number {
  if (error instanceof ConfigurationError) return ERROR_CODES.CONFIGURATION_ERROR;
...
export function assertConfig(condition: boolean, message: string): asserts condition {
  if (!condition) {
    throw new ConfigurationError(message);
  }
}

export function assertTemplate(condition: boolean, message: string, templateName?: string): asserts condition {
  if (!condition) {
    throw new TemplateError(message, templateName);
  }
}

export function assertFileSystem(condition: boolean, message: string, path?: string): asserts condition {
  if (!condition) {
