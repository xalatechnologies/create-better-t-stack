...
import { fileExists, processTemplate, readFile } from "./fs.js";
...
export class TemplateEngine {
	...
	private registerBuiltInHelpers(): void {
		...
		this.registerHelper("langCode", (locale: string) => locale.split("-")[0]);
	}
	...
	private processLoops(template: string, context: TemplateContext): string {
		...
		return template.replace(eachRegex, (match, arrayPath, content) => {
			...
			if (!Array.isArray(array)) {
				logger.warn(
					`Expected array for {{#each ${arrayPath}}}, got ${typeof array}`,
				);
				return "";
			}
	...
	private evaluateExpression(
		...
		try {
			...
			if (trimmed.includes("===")) {
				...
				return (
					this.getValueFromPath(left, context) ===
					this.getValueFromPath(right, context)
				);
			}
		...
		for (const [name, varDef] of Object.entries(metadata.variables)) {
			...
			if (varDef.options && varDef.type === "select") {
				...
				if (!validOptions.includes(value)) {
					errors.push(
						`Variable "${name}" must be one of: ${validOptions.join(", ")}`,
					);
				}
			}
