	...
	implements IValidator
{
	...
	private cache = new Map<
		string,
		{ result: ComplianceCheckResult; timestamp: number }
	>();
	...
	protected async doInitialize(): Promise<void> {
		...
		try {
			...
			serviceLogger.info(
				"Norwegian compliance service initialized successfully",
			);
		} catch (error) {
			serviceLogger.error(
				"Failed to initialize compliance service:",
				error as Error,
			);
			throw error;
		}
	...
	async validate(input: IValidationInput): Promise<IValidationResult> {
		...
		try {
			serviceLogger.info(
				`Starting Norwegian compliance validation for ${input.files.size} files`,
			);
			...
			serviceLogger.info(
				`Compliance validation completed in ${duration.toFixed(2)}ms`,
			);
			...
			const duration = performance.now() - startTime;
			serviceLogger.error(
				`Compliance validation failed after ${duration.toFixed(2)}ms:`,
				error as Error,
			);
			throw error;
		}
	...
	async autofix(result: IValidationResult): Promise<{
		...
		try {
			...
			serviceLogger.info(
				`Auto-fix completed: ${issuesFixed} issues fixed, ${filesModified} files modified`,
			);
	...
	async checkCompliance(
	...
	): Promise<ComplianceServiceResult> {
		this.validateServiceState();
		...
		try {
			...
			if (request.classification && request.projectName) {
				const tempConfig = createDefaultNorwegianConfig(
					request.projectName,
	...
	private async loadConfiguration(): Promise<void> {
		if (
			...
			(await fileExists(this.options.configPath))
		) {
			try {
				const configContent = await readFile(this.options.configPath);
				this.config = JSON.parse(configContent);
				logger.debug(
					`Loaded compliance configuration from: ${this.options.configPath}`,
				);
			} catch (error) {
				logger.warn(
					`Failed to load compliance configuration, using default:`,
					error,
				);
	...
	private async saveConfiguration(): Promise<void> {
		...
		try {
			...
			logger.debug(
				`Saved compliance configuration to: ${this.options.configPath}`,
			);
		} catch (error) {
	...
	private async ensureOutputDirectory(): Promise<void> {
		...
		try {
			...
			logger.debug(
				`Output directory already exists or creation failed: ${this.options.outputDir}`,
			);
		}
	}
	...
	private convertToValidationResult(
		...
		return {
			...
			metrics: {
				...
				errorCount: checkResult.issues.filter((i) => i.severity === "critical")
					.length,
	...
	private async generateReports(
		...
		if (
			!this.options.reportFormats ||
			this.options.reportFormats.length === 0
		) {
			return;
		}
	...
	private async generateSpecificReports(
		...
		for (const format of formats) {
			try {
				const reportContent = await this.checker!.generateComplianceReport(
					checkResult,
					format,
				);
				...
				logger.debug(
					`Generated ${format.toUpperCase()} compliance report: ${filePath}`,
				);
			} catch (error) {
