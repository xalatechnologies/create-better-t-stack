 ...
 */
export function getMinimumWCAGLevel(
	classification: NSMClassification,
): WCAGLevel {
...
export function generateComplianceMetadata(
	...
	if (componentType) {
		const complianceReqs = getComponentComplianceRequirements(
			componentType,
			classification,
		);
		return {
			...metadata,
			requirements: [
				...complianceReqs.requirements,
				...complianceReqs.additionalChecks,
			],
		};
	}
...
export function validateComplianceConfiguration(config: any): {
	...
	if (!config.project?.dataController)
		missingFields.push("project.dataController");
	if (!config.project?.privacyOfficer)
		missingFields.push("project.privacyOfficer");
	...
	if (!config.localization?.defaultLocale)
		missingFields.push("localization.defaultLocale");
	if (!config.localization?.supportedLocales?.length)
		missingFields.push("localization.supportedLocales");
	...
	if (!config.localization?.supportedLocales?.includes("nb-NO")) {
		warnings.push(
			"Norwegian Bokmål (nb-NO) should be supported for Norwegian compliance",
		);
	}
...
export const COMPLIANCE_MODULE_INFO = {
	...
	version: "1.0.0",
	description:
		"Comprehensive Norwegian compliance system for NSM, GDPR, and WCAG standards",
