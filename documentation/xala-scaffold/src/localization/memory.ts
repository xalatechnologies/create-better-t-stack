...
export class TranslationMemory {
	...
	search(
		...
		return results
			.sort(
				(a, b) =>
					this.calculateScore(b, options) - this.calculateScore(a, options),
			)
	...
	getStatistics(): {
		...
		return {
			...
			averageQuality:
				this.entries.size > 0 ? totalQuality / this.entries.size : 0,
			topDomains,
		};
	}
