...
// Get locale metadata
export function getLocaleMetadata(locale: LocaleCode): LocaleMetadata | undefined {
...
export function getLocaleFallbackChain(
  ...
  if (config.defaultLocale !== locale && config.defaultLocale !== config.fallbackLocale) {
    ...
    if (config.namespaceStrategy === 'nested') {
      ...
      if (!(await fileExists(defaultFile))) {
        await saveLocaleFile(defaultFile, {
          ...
          translations: {
            common: {
              welcome: locale === 'nb-NO' ? 'Velkommen' :
                      locale === 'nn-NO' ? 'Velkomen' :
                      locale === 'en-US' ? 'Welcome' :
                      locale === 'ar-SA' ? 'مرحبا' :
                      locale === 'fr-FR' ? 'Bienvenue' : 'Welcome',
            },
          },
        });
      }
