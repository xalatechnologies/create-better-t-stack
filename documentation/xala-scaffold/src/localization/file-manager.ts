...
export class TranslationFileManager {
  ...
  async loadProjectTranslations(
    ...
    for (const locale of config.supportedLocales) {
      const filePath = getLocaleFilePath(projectPath, locale, undefined, config);
  ...
  async findMissingTranslations(
    ...
    const translations = await this.loadProjectTranslations(projectPath, config);
  ...
  async findUnusedTranslations(
    ...
    for (const [locale, data] of translations) {
      ...
      const unusedKeys = allKeys.filter(key => 
        !usedKeys.has(key) && !key.startsWith('_metadata.')
      );
  ...
  async cleanUnusedTranslations(
    ...
    const unused = await this.findUnusedTranslations(projectPath, usedKeys, config);
    ...
    logger.info(`Removed ${Array.from(unused.values()).flat().length} unused translations`);
  }
  ...
  async mergeTranslations(
    sourceFile: string,
    targetFile: string,
    strategy: 'overwrite' | 'keep-existing' | 'merge-metadata' = 'keep-existing'
  ): Promise<void> {
