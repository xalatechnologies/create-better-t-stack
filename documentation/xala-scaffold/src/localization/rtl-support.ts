import { LocaleCode, isRTLLocale, getLocaleMetadata } from './core.js';
...
`;
  ...
  needsFlipping(property: string): boolean {
    return property in RTL_PROPERTIES || property in (this.config.customMappings || {});
  }
  ...
  generateFrameworkConfig(framework: 'next' | 'react' | 'vue'): Record<string, any> {
    switch (framework) {
      case 'next':
        return {
          i18n: {
  ...
  validateRTLImplementation(
    ...
    if (componentCode.includes('left:') || componentCode.includes('right:')) {
      issues.push('Found hardcoded left/right positioning - use logical properties');
    }
    ...
    if (componentCode.includes('text-align: left') || componentCode.includes('text-align: right')) {
...
export function convertTailwindForRTL(classes: string, locale: LocaleCode): string {
