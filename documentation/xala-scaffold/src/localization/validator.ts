...
import { LocaleCode, LocalizationConfig, TranslationFile } from "./core.js";
...
export interface ValidationOptions {
	...
	customValidators?: Array<
		(key: string, value: string, locale: LocaleCode) => ValidationError | null
	>;
}
...
export class TranslationValidator {
	...
	private validateLocale(
		...
		for (const key of allKeys) {
			...
			if (this.options.checkPlaceholders) {
				const placeholderError = this.validatePlaceholders(
					key,
					value,
					refValue,
				);
	...
	private validatePlaceholders(
		...
		const missing = Array.from(refPlaceholders).filter(
			(p) => !valuePlaceholders.has(p),
		);
		const extra = Array.from(valuePlaceholders).filter(
			(p) => !refPlaceholders.has(p),
		);
	...
	private createResult(
		...
		const validKeys =
			totalKeys - errors.filter((e) => e.type === "missing").length;
	...
	generateReport(result: ValidationResult): string {
		...
		for (const [locale, summary] of result.summary.localeBreakdown) {
			...
			report.push(
				`- Translated: ${summary.translatedKeys}/${summary.totalKeys}`,
			);
		...
		if (result.errors.length > 0) {
			...
			for (const error of result.errors) {
				report.push(
					`- [${error.severity.toUpperCase()}] ${error.locale} - ${error.key}: ${error.message}`,
				);
			}
		}
