...
export class TranslationKeyGenerator {
  ...
  constructor(options: Partial<KeyGenerationOptions> = {}) {
    this.options = { ...DEFAULT_OPTIONS, ...options } as Required<KeyGenerationOptions>;
  }
  ...
  private getNamespace(filePath: string): string {
    ...
    // Limit depth
    return parts.slice(0, 2).map(p => this.formatKeyPart(p)).join(this.options.separator);
  }
  ...
  private resolveCollision(baseKey: string, text: ExtractedText, index: number): string {
...
export const KEY_NAMING_CONVENTIONS = {
  ...
  // Form keys
  formField: (form: string, field: string) => `forms.${form}.fields.${field}.label`,
