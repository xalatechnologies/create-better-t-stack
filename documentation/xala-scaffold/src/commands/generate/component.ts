...
function parseProp(propString: string): PropDefinition {
	...
	if (!match) {
		throw new ValidationError(`Invalid prop format: "${propString}"`, [
...
function validateComponentName(name: string): void {
	if (!/^[A-Z][A-Za-z0-9]*$/.test(name)) {
		throw new ValidationError("Invalid component name", [
			{
				field: "name",
				message:
...
export async function generateComponent(
	...
	try {
		...
		await writeFile(path.join(componentDir, `${name}.tsx`), componentContent);
...
`;
		await writeFile(path.join(componentDir, "index.ts"), indexContent);

		// Generate test file
		if (config.hasTests) {
			const testContent = generateTestContent(config);
			await writeFile(path.join(componentDir, `${name}.test.tsx`), testContent);
		...
		logger.info("\nNext steps:");
		logger.info(
			`1. Import and use: import { ${name} } from '@/components/${config.type}/${name}';`,
		);
		if (config.hasTests) {
