...
export async function registerCommands(program: Command): Promise<void> {
  ...
  try {
    // Import init command
    const { default: initCommand } = await import('./init.js').catch(() => ({ default: null }));
    if (initCommand) registerCommand(initCommand);
    
    // Import migrate command
    const { default: migrateCommand } = await import('./migrate.js').catch(() => ({ default: null }));
    if (migrateCommand) registerCommand(migrateCommand);
    
    // Import generate command with subcommands
    const { default: generateCommand, registerGenerateSubcommands } = await import('./generate/index.js').catch(() => ({ default: null, registerGenerateSubcommands: null }));
    if (generateCommand) {
