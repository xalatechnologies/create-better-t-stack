...
async function promptForConfig(options: InitOptions): Promise<ProjectConfig> {
		...
		: await inquirer.prompt([
				{
					...
					validate: (input) =>
						input.length > 0 || "At least one locale must be selected",
				},
			]);
	...
	// CI/CD
	const cicd = features.includes("cicd") ? await promptForCICD() : undefined;
...
async function executeInit(options: InitOptions): Promise<void> {
	...
	const projectConfig =
		options.skipPrompts && options.name && options.platform
			? {
					name: options.name,
