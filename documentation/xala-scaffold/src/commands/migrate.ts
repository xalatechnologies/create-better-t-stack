...
type MigrationSource = 'lovable' | 'bolt' | 'vercel' | 'create-react-app' | 'custom';
...
// Get migration configuration
async function getMigrationConfig(options: MigrateOptions): Promise<MigrationConfig> {
  ...
  const { features } = await inquirer.prompt([{
    ...
    choices: [
      { name: 'Extract text for localization', value: 'localization', checked: true },
      { name: 'Add Norwegian compliance', value: 'compliance', checked: true },
      { name: 'Generate tests', value: 'tests', checked: true },
      { name: 'Convert to TypeScript', value: 'typescript', checked: true },
      { name: 'Add accessibility features', value: 'accessibility', checked: true },
    ],
  }]);
...
const migrateCommand: CommandMetadata = {
  ...
  options: [
    {
      flags: '--from <platform>',
      description: 'Source platform (lovable, bolt, vercel, create-react-app, custom)',
    },
    {
      flags: '-s, --source <path>',
