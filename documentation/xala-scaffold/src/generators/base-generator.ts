...
import { TemplateContext, TemplateEngine } from "../utils/template.js";
...
export abstract class BaseGenerator {
	...
	protected async processTemplate(
		...
		try {
			...
			if (templateFile.isTemplate) {
				content = this.templateEngine.processTemplateString(
					content,
					context.variables,
				);
			}
			...
			if (this.config.hooks.beforeFileWrite) {
				content = await this.config.hooks.beforeFileWrite(
					outputPath,
					content,
					context,
				);
	...
	protected resolveOutputPath(
		...
		cleanPath = this.templateEngine.processTemplateString(
			cleanPath,
			context.variables,
		);
	...
	protected async validateGeneratedFile(
		filePath: string,
		content: string,
	): Promise<void> {
	...
	protected async scanTemplateDirectory(
		templateDir: string,
		...
		async function scanDir(dir: string, baseDir: string): Promise<void> {
			...
			for (const entry of entries) {
				...
				if (entry.isDirectory()) {
					...
					const isTemplate = templateExtensions.some((ext) =>
	...
	protected async loadTemplateFiles(
...
export function groupResultsByTemplate(
	results: GenerationResult[],
