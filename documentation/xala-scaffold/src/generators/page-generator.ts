...
const PageGenerationInputSchema = z.object({
	...
	layout: z
		.enum(["default", "auth", "dashboard", "minimal"])
		.default("default"),
	authentication: z
		.object({
...
export class PageGenerator extends BaseGenerator {
		...
		super({
			...
			hooks: {
				beforeGenerate: async (context) => {
					logger.info(
						`Generating ${context.variables.platform} page: ${context.variables.name}`,
					);
				},
				afterGenerate: async (context, results) => {
					const successCount = results.filter((r) => r.success).length;
					logger.info(
						`Generated ${successCount} files for page ${context.variables.name}`,
					);
				},
			},
		});
	...
	private generateImports(input: PageGenerationInput): string[] {
		...
		if (input.layout !== "minimal") {
			imports.push(
				`import { ${input.layout === "default" ? "Default" : input.layout}Layout } from '../layouts';`,
			);
		}
	...
	private generateTypes(input: PageGenerationInput): string[] {
		...
		if (input.typescript) {
			...
			// Data fetching types
			if (
				input.dataFetching.type === "ssr" ||
				input.dataFetching.type === "ssg"
			) {
				if (input.platform === "nextjs") {
					types.push(`
interface PageData {
  ...
  const handlePageChange = (page: number) => {
	...
	private generateSEOData(input: PageGenerationInput): SEOData {
		...
		const description =
			input.seo.description || `${input.name} page description`;
