...
import { LocalizationConfig, DEFAULT_LOCALIZATION_CONFIG } from '../localization/core.js';
...
const ProjectGenerationInputSchema = z.object({
  ...
  styling: z.enum(['tailwind', 'styled-components', 'css-modules', 'vanilla']).default('tailwind'),
...
export class ProjectGenerator extends BaseGenerator {
  constructor() {
    super({
      ...
      description: 'Generate new project structure with platform-specific configuration',
      ...
      hooks: {
        beforeGenerate: async (context) => {
          logger.info(`Generating ${context.variables.platform} project: ${context.variables.name}`);
        },
        afterGenerate: async (context, results) => {
          const successCount = results.filter(r => r.success).length;
          logger.info(`Generated ${successCount} files for project ${context.variables.name}`);
        },
      },
    });
  }
  ...
  async validateInput(input: any): Promise<ProjectGenerationInput> {
    ...
    if (await fileExists(validated.outputPath)) {
      const entries = await import('fs').then(fs => fs.promises.readdir(validated.outputPath));
      if (entries.length > 0) {
        throw new Error(`Output directory ${validated.outputPath} is not empty`);
      }
    }
  ...
  async prepareContext(input: ProjectGenerationInput): Promise<GenerationContext> {
  ...
  async getTemplateFiles(): Promise<TemplateFile[]> {
    const templateFiles = await this.scanTemplateDirectory(this.config.templateDir);
  ...
  private generateDependencies(input: ProjectGenerationInput): {
    ...
    for (const [feature, enabled] of Object.entries(input.features)) {
      if (enabled && FEATURE_DEPENDENCIES[feature as keyof typeof FEATURE_DEPENDENCIES]) {
        const featureDeps = FEATURE_DEPENDENCIES[feature as keyof typeof FEATURE_DEPENDENCIES];
  ...
  private generateScripts(input: ProjectGenerationInput): Record<string, string> {
    ...
    switch (input.platform) {
      ...
      case 'library':
        ...
        scripts.lint = 'eslint src --ext ts --report-unused-disable-directives --max-warnings 0';
        break;
    }
    ...
    if (input.compliance.nsm || input.compliance.gdpr) {
      scripts['validate:compliance'] = 'npx enterprise-standards validate --norwegian-compliance';
  ...
  // Generate configuration files content
  private async generateConfigurations(input: ProjectGenerationInput): Promise<Record<string, any>> {
      ...
      jobs: {
        test: {
          ...
          steps: [
            ...
            input.features.testing && { run: `${input.packageManager} run test:run` },
            { run: `${input.packageManager} run build` },
            input.compliance.nsm && { run: `${input.packageManager} run validate:compliance` },
          ].filter(Boolean),
        },
      },
    };
...
export async function generateProject(input: ProjectGenerationInput): Promise<void> {
