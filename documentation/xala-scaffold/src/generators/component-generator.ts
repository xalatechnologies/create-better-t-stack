...
const ComponentGenerationInputSchema = z.object({
	name: z
		.string()
		.min(1)
		.refine((name) => /^[A-Z][a-zA-Z0-9]*$/.test(name), {
			message: "Component name must be PascalCase",
		}),
	type: z
		.enum(["display", "form", "layout", "business", "composite", "hook"])
		.default("display"),
	props: z
		.array(
			z.object({
...
export class ComponentGenerator extends BaseGenerator {
	constructor() {
		super({
			...
			description:
				"Generate React components with TypeScript, tests, and stories",
			...
			hooks: {
				beforeGenerate: async (context) => {
					logger.info(
						`Generating ${context.variables.type} component: ${context.variables.name}`,
					);
				},
				afterGenerate: async (context, results) => {
					const successCount = results.filter((r) => r.success).length;
					logger.info(
						`Generated ${successCount} files for component ${context.variables.name}`,
					);
				},
			},
		});
	...
	private generateImports(input: ComponentGenerationInput): string[] {
		...
		if (
			input.accessibility.keyboardNavigation ||
			input.accessibility.focusManagement
		) {
			imports.push(
				"import { useKeyboard, useFocus } from '@xala-technologies/ui-system';",
			);
		}
		...
		if (input.props.length > 0) {
			imports.push(
				`import type { ${input.name}Props } from './${input.name}.types';`,
			);
		}
...
test('supports keyboard navigation', async () => {
  ...
  // Test focus management
	...
	private generateLocalizationFeatures(
		input: ComponentGenerationInput,
