...
export class DatabaseSchemaGenerator extends BaseGenerator {
	...
	async generateFromDescription(
		...
		try {
			this.logger.info("Generating database schema from description", {
				description,
			});
	...
	async generateSchema(
		options: IDatabaseSchemaOptions,
	): Promise<ISchemaGenerationResult> {
		try {
...
5. Compliance requirements (GDPR, audit trail, etc.)`;
	...
	private async generateEntities(
		...
		for (const entityData of analysis.entities || []) {
			const entity: IEntityDefinition = {
				...
				constraints: [],
				audit:
					entityData.requiresAudit || options.features?.includes("audit-trail"),
				multiTenant:
					entityData.multiTenant || options.features?.includes("multi-tenant"),
				softDelete:
					entityData.softDelete || options.features?.includes("soft-delete"),
			};
	 ...
	 * Classify GDPR data
	 */
	private classifyGDPRData(
		fieldName: string,
		fieldType: string,
	): IFieldDefinition["gdprCategory"] {
				...
				if (
					...
					const targetEntity = entities.find(
						(e) =>
							...
							e.name.toLowerCase() ===
								this.toPascalCase(targetName).toLowerCase(),
					);
	...
	private isJunctionTable(
		entity: IEntityDefinition,
		allEntities: IEntityDefinition[],
	): boolean {
		const foreignKeys = entity.fields.filter(
			(f) => f.name.endsWith("Id") && f.name !== "id" && f.name !== "tenantId",
		);
	...
	private applyRelationships(
		entities: IEntityDefinition[],
		relationships: IRelationDefinition[],
	): void {
	...
	private async optimizeIndexes(entities: IEntityDefinition[]): Promise<void> {
		for (const entity of entities) {
			...
			const foreignKeys = entity.fields.filter(
				(f) => f.name.endsWith("Id") && f.name !== "id",
			);
	...
	private applyFeatures(
		entities: IEntityDefinition[],
		features: IDatabaseSchemaOptions["features"],
	): void {
		for (const entity of entities) {
	...
	private async generateSchemaFiles(
		...
		if (
			options.output?.format === "migrations" ||
			options.output?.format === "all"
		) {
...
`;
	 ...
	 */
	private generatePrismaEncryptionExtension(
		entities: IEntityDefinition[],
	): string {
...
${entities
	...
	.map(
		(e) => `    ${e.name}: [${e.encrypted!.map((f) => `'${f}'`).join(", ")}]`,
	)
	.join(",\n")}
  };
...
`;
		...
		const indexContent = entities
			.map((e) => `export * from './${this.toKebabCase(e.name)}';`)
			.join("\n");
...
${entity.fields
	.filter((f) => f.name !== "id")
	.map((field) => this.generateMongooseField(field))
	.join(",\n")}
}, {
  timestamps: true,
...
`;
	...
	private generateMongooseField(field: IFieldDefinition): string {
		...
		if (field.validation?.length) {
			const validators = field.validation
				.map((rule) => {
					switch (rule.type) {
						case "min":
  ...
  return decrypted;
		...
		for (const entity of entities) {
			...
			if (entity.indexes) {
				for (const index of entity.indexes) {
					const indexName =
						index.name ||
						`idx_${this.toSnakeCase(entity.name)}_${index.fields.join("_")}`;
	...
	private generateSQLColumn(
		...
		switch (field.type) {
			...
			case "date":
				column +=
					database === "postgresql" ? "TIMESTAMP WITH TIME ZONE" : "TIMESTAMP";
				break;
			case "json":
...
`;
	 ...
	 */
	private generateTypeORMMigration(
		entities: IEntityDefinition[],
		timestamp: string,
	): string {
...
${entities
	.map(
		(entity) => `    await queryRunner.createTable(
      new Table({
...
${
	entity.indexes
		?.map(
			(index) => `    await queryRunner.createIndex(
...
${entities
	...
	private generateTypeORMMigrationColumn(field: IFieldDefinition): string {
		...
		if (field.default !== undefined) {
			column.default =
				field.default === "now()" ? "CURRENT_TIMESTAMP" : field.default;
		}
	 ...
	 */
	private async generateNorwegianTestData(
		entities: IEntityDefinition[],
	): Promise<any> {
...
${Object.entries(seedData)
  ...
  console.log('✅ Created ${records.length} ${entity} records');`,
	)
	.join("\n")}
...
`;
	...
	private validateOptions(options: IDatabaseSchemaOptions): void {
		...
		for (const entity of options.entities) {
			...
			for (const field of entity.fields) {
				...
				if (!field.type) {
					throw new Error(
						`Field type is required for ${field.name} in entity ${entity.name}`,
					);
				}
			}
		}
	...
	private async validateSchema(
		...
		for (const entity of entities) {
			...
			for (const fk of foreignKeys) {
				...
				if (!hasIndex) {
					warnings.push(
						`Missing index on foreign key ${entity.name}.${fk.name}`,
					);
				}
			}
		}
		...
		for (const entity of entities) {
			const personalData = entity.fields.filter(
				(f) => f.gdprCategory === "personal" || f.gdprCategory === "sensitive",
			);

			if (personalData.length > 0 && !entity.audit) {
				warnings.push(
					`Entity ${entity.name} contains personal data but audit trail is not enabled`,
				);
			}

			for (const field of personalData) {
				if (
					options.compliance?.nsmClassification !== "OPEN" &&
					!field.encrypted
				) {
					errors.push(
						`Personal data field ${entity.name}.${field.name} should be encrypted for NSM classification ${options.compliance?.nsmClassification}`,
					);
				}
			}
		}
		...
		for (const entity of entities) {
			...
			for (const field of entity.fields) {
				if (!/^[a-z][a-zA-Z0-9]*$/.test(field.name)) {
					warnings.push(
						`Field name ${entity.name}.${field.name} should be in camelCase`,
					);
				}
			}
		}
	...
	private checkGDPRCompliance(entities: IEntityDefinition[]): boolean {
		for (const entity of entities) {
			const hasPersonalData = entity.fields.some(
				(f) => f.gdprCategory === "personal" || f.gdprCategory === "sensitive",
			);

			if (hasPersonalData) {
				// Check for required GDPR fields
				const hasConsent = entity.fields.some(
					(f) =>
				...
				const sensitiveFields = entity.fields.filter(
					(f) => f.gdprCategory === "sensitive",
				);
