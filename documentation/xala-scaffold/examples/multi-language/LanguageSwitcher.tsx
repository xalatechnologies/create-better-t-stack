...
export const LanguageSwitcher: React.FC<LanguageSwitcherProps> = ({
  ...
  const { t, locale, setLocale, isRTL, formatDate, formatNumber } = useTranslation();
  ...
  const handleKeyDown = useCallback((event: React.KeyboardEvent) => {
    ...
    switch (event.key) {
      ...
      default:
        ...
        const matchIndex = SUPPORTED_LANGUAGES.findIndex((lang, index) => 
          index > focusedIndex && 
  ...
  const handleLanguageSelect = useCallback(async (newLocale: LocaleCode) => {
    if (newLocale === locale) {
      setIsOpen(false);
      return;
    }
    ...
    const selectedLanguage = SUPPORTED_LANGUAGES.find(lang => lang.code === newLocale);
  ...
  }, [locale, setLocale, onLanguageChange, auditLogger, classification, currentLanguage, t]);
  ...
  const formatExampleContent = () => {
    ...
    return {
      ...
      currency: formatNumber(sampleNumber, {
        style: 'currency',
        currency: locale.startsWith('nb-') || locale.startsWith('nn-') ? 'NOK' : 
                 locale === 'ar-SA' ? 'SAR' : 
                 locale === 'fr-FR' ? 'EUR' : 'USD',
      }),
    };
  };
          ...
          >
              ...
              {SUPPORTED_LANGUAGES.map((language, index) => (
                <button
                  ...
                  className={`w-full px-4 py-3 text-left hover:bg-gray-100 focus:bg-gray-100 focus:outline-none ${
                    language.code === locale ? 'bg-blue-50 text-blue-700' : 'text-gray-900'
            ...
            <div className="border-t border-gray-200 p-4 bg-gray-50">
              ...
              <div className="space-y-1 text-xs text-gray-600">
                <div>
                  <strong>{t('languageSwitcher.examples.date')}:</strong> {exampleContent.date}
                </div>
                <div>
                  <strong>{t('languageSwitcher.examples.time')}:</strong> {exampleContent.time}
                </div>
                <div>
                  <strong>{t('languageSwitcher.examples.number')}:</strong> {exampleContent.number}
                </div>
                <div>
                  <strong>{t('languageSwitcher.examples.currency')}:</strong> {exampleContent.currency}
                </div>
              </div>
            </div>
          </div>
