...
import { NSMClassification, GDPRDataCategory } from '@/types/compliance';
...
const userProfileFormSchema = z.object({
  ...
  gdprConsent: z.object({
    dataProcessing: z.boolean()
      .refine(val => val === true, 'userProfile.validation.gdprDataProcessingRequired'),
    marketing: z.boolean(),
    analytics: z.boolean(),
...
export const UserProfileForm: React.FC<UserProfileFormProps> = ({
  ...
  const validatePersonalId = useCallback(async (personalId: string) => {
      ...
      <div className="compliance-status mb-6 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="flex space-x-4 text-sm">
          <div className="flex items-center">
            <span className={`w-2 h-2 rounded-full mr-2 ${complianceStatus.nsm ? 'bg-green-500' : 'bg-red-500'}`} />
            NSM {classification}
          </div>
          <div className="flex items-center">
            <span className={`w-2 h-2 rounded-full mr-2 ${complianceStatus.gdpr ? 'bg-green-500' : 'bg-red-500'}`} />
            GDPR
          </div>
          <div className="flex items-center">
            <span className={`w-2 h-2 rounded-full mr-2 ${complianceStatus.wcag ? 'bg-green-500' : 'bg-red-500'}`} />
            WCAG AAA
          </div>
        </div>
      </div>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <AccessibleFormField
                    ...
                    type="text"
                    error={errors.personalId?.message ? t(errors.personalId.message) : undefined}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <NorwegianPostalCodeInput
                    ...
                    error={errors.address?.postalCode?.message ? t(errors.address.postalCode.message) : undefined}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <AccessibleFormField
                    ...
                    type="text"
                    required
                    error={errors.address?.municipality?.message ? t(errors.address.municipality.message) : undefined}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <GDPRConsentCheckbox
                    ...
                    description={t('userProfile.gdpr.dataProcessing.description')}
                    required
                    error={errors.gdprConsent?.dataProcessing?.message ? t(errors.gdprConsent.dataProcessing.message) : undefined}
                    disabled={readOnly}
                    {...field}
                  />
                )}
