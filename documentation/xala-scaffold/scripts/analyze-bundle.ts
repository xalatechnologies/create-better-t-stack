...
async function analyzeBundle() {
  ...
  try {
    ...
    console.log('─'.repeat(50));
    console.log(`${'Total'.padEnd(28)} ${formatBytes(totalSize).padStart(10)} ${formatBytes(totalGzipSize).padStart(10)}`);
    ...
    console.log('\n📏 Size Budget Check:');
    console.log(`  Max size: ${formatBytes(maxSize)} - ${totalSize <= maxSize ? '✅ PASS' : '❌ FAIL'}`);
    console.log(`  Max gzip: ${formatBytes(maxGzipSize)} - ${totalGzipSize <= maxGzipSize ? '✅ PASS' : '❌ FAIL'}`);
