...
export function registerXalaCommands(program: Command, extension: XalaExtension) {
...
import { Command } from 'commander';
import { intro, outro, text, select, multiselect, spinner, confirm, isCancel, cancel } from '@clack/prompts';
  ...
  program
    ...
    .action(async (projectName, options) => {
      ...
      const themePreset = await select({
        ...
        options: [
          { value: 'default', label: 'Xala Default', hint: 'Blue & orange theme' },
          { value: 'government', label: 'Government', hint: 'Official Norwegian colors' },
          { value: 'healthcare', label: 'Healthcare', hint: 'Calming blues and greens' },
          { value: 'finance', label: 'Finance', hint: 'Professional dark theme' },
...
import { Command } from 'commander';
import { intro, outro, text, select, confirm, multiselect, isCancel, cancel } from '@clack/prompts';
import chalk from 'chalk';
import path from 'path';
import fs from 'fs-extra';

export function addXalaComponentCommand(program: Command, extension: XalaExtension) {
  program
    ...
    .action(async (componentName, options) => {
      ...
      // Component type
      const componentType = options.type || await select({
        message: 'Component type:',
        options: [
          { value: 'ui', label: 'UI Component', hint: 'Reusable UI element' },
          { value: 'feature', label: 'Feature Component', hint: 'Business logic component' },
          { value: 'layout', label: 'Layout Component', hint: 'Page layout structure' },
          { value: 'form', label: 'Form Component', hint: 'Form with validation' },
          { value: 'chat', label: 'Chat Component', hint: 'Chat interface element' },
        ],
      });
      ...
      const features = await multiselect({
        ...
        options: [
          { value: 'typescript', label: 'TypeScript', hint: 'Type definitions' },
...
function generateComponentTemplate(config: ComponentConfig): string {
  ...
  if (config.type === 'form' && config.xalaFeatures.formFields) {
    if (config.xalaFeatures.formFields.includes('personal-number')) {
      imports.push(`import { PersonalNumberInput } from '@xala-technologies/ui-system';`);
    }
    if (config.xalaFeatures.formFields.includes('org-number')) {
      imports.push(`import { OrganizationNumberInput } from '@xala-technologies/ui-system';`);
    }
    if (config.xalaFeatures.formFields.includes('classification')) {
      imports.push(`import { ClassificationSelector } from '@xala-technologies/ui-system';`);
    }
  }
